...
class RashiListExport implements FromView, ShouldAutoSize, WithStyles,WithColumnWidths ,WithHeadings, WithEvents
{
    use Exportable;
    protected $data;
    ...
    {
        return view('file-exports.rashi-list', [
            'data' => $this->data,
        ]);
    }
    ...
    public function styles(Worksheet $sheet) {
        ...
        $sheet->getStyle('F5:F'.$this->data['rashi']->count() + 4)->getFill()->applyFromArray([
            'fillType' => 'solid',
            'rotation' => 0,
            'color' => ['rgb' => 'FFF9D1'],
        ]);
    ...
    public function setImage($workSheet) {
        $this->data['rashi']->each(function($item,$index) use($workSheet) {
            $drawing = new Drawing();
            $drawing->setName($item->name);
            $drawing->setDescription($item->name);
            $drawing->setPath(file_exists(storage_path('app/public/rashi/'.$item->image))? storage_path('app/public/rashi/'.$item->image) : public_path('assets/back-end/img/rashi.png'));
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function(AfterSheet $event) {
                ...
                $event->sheet->getStyle('A4:F'.$this->data['rashi']->count() + 4) // Adjust the range as per your needs
                    ->getAlignment()
                    ->setHorizontal(Alignment::HORIZONTAL_CENTER)
                    ->setVertical(Alignment::VERTICAL_CENTER);
