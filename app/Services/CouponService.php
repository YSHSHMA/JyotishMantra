...
class CouponService
{
    ...
    public function getCouponData($request, $addedBy): array
    {
        ...
        $isDiscountTypeValid = in_array($request['coupon_type'], $validCouponTypes);
        ...
        return [
            ...
            'discount_type' => $isDiscountTypeValid ? $request['discount_type'] : 'amount',
            'discount' => $isDiscountTypeValid ? ($request['discount_type'] == 'amount' ? currencyConverter(amount: $request['discount']) : $request['discount']) : 0,
            'max_discount' => $isDiscountTypeValid && $request['discount_type'] == 'percentage'
                ? currencyConverter(amount: $request['max_discount'] != null ? $request['max_discount'] : $request['discount'])
                : 0,
            'created_at' => now(),
            'updated_at' => now(),
        ];
    ...
    public function getAddData(object $request): array
    {
        ...
        if (
            $request['coupon_type'] == 'discount_on_purchase' ||
            $request['coupon_type'] == 'free_delivery' ||
            $request['coupon_type'] == 'pooja' ||
            $request['coupon_type'] == 'vippooja' ||
            $request['coupon_type'] == 'anushthan' ||
            $request['coupon_type'] == 'counselling' ||
            $request['coupon_type'] == 'event_order' ||
            $request['coupon_type'] == 'tour_order' ||
            $request['coupon_type'] == 'offlinepooja'
        ...
        if (
            $request['coupon_type'] == 'discount_on_purchase' ||
            $request['coupon_type'] == 'first_order' ||
            $request['coupon_type'] == 'pooja' ||
            $request['coupon_type'] == 'vippooja' ||
            $request['coupon_type'] == 'anushthan' ||
            $request['coupon_type'] == 'counselling' ||
    ...
    {
        if (
            ($request['coupon_type'] == 'discount_on_purchase' ||  $request['coupon_type'] == 'pooja' ||
                $request['coupon_type'] == 'special_pooja' ||  $request['coupon_type'] == 'vippooja' ||
                $request['coupon_type'] == 'anushthan' ||  $request['coupon_type'] == 'offlinepooja' ||
                $request['coupon_type'] == 'counselling') && ($request['discount_type'] == 'amount' && $request['discount'] > $request['min_purchase'])
        ) {
