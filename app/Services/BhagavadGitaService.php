...
class BhagavadGitaService
{
    ...
    {
        $image = $request->file('image') ? $this->update('sahitya/bhagavad-gita/', $data['image'],'webp', $request->file('image')) : $data['image'];
        return [
            'name' => $request->name[array_search('en', $request['lang'])],
            'image' => $image,
        ];
    ...
    {
        if ($data['image']) {$this->delete('sahitya/bhagavad-gita/'.$data['image']);};
        return true;
    }
