...
trait Payment
{
    public static function generate_link(object $payer, object $payment_info, Object $receiver)
    {
        ...
        $payment->previous_url = url()->previous();
        if (isset($payment_info->getAdditionalData()['expiration']) && !empty($payment_info->getAdditionalData()['expiration'])) {
            $payment->expires = $payment_info->getAdditionalData()['expiration'];
        }
        $payment->save();
        ...
        } else if ($payment->payment_method == 'razor_paywithexpir') {
            $expiration_time = urlencode($payment_info->getAdditionalData()['expiration']);
            return url("payment/razor-pay/pay/?payment_id={$payment->id}&expires={$expiration_time}");
