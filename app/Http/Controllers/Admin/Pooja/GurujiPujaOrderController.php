...
class GurujiPujaOrderController extends Controller
{
    use PdfGenerator;

    public function orders_list($status, Request $request)
    {
        $ordersQuery = Service_order::where('indivisual',1)->where('type', 'panditpooja')->where('is_block', '!=', 9)->with(['leads', 'packages', 'services', 'customers', 'pandit']);
        if ($status !== 'all') {
        ...
        return view('admin-views.pooja.panditpuja.list', compact('orders', 'users','paymentPublishedStatus','paymentGatewayPublishedStatus','payment_gateways_list','digital_payment'));
    }
    ...
    {
        $serviceId = Service_order::select('service_id')->where('id', $id)->where('type', 'panditpooja')->where('indivisual',1)->first()->service_id;
        $details = Service_order::where('type', 'panditpooja')->where('indivisual',1)->where('id', $id)->with(['leads','customers','services','panditpackage','packages','payments','astrologer','product_leads.productsData'])->first();
        $details['pooja_pandit'] = Astrologer::where('id', $details['pandit_assign'])->where('is_pandit_pooja', 'like', '%' . $details['service_id'] . '%')->where('status', 1)->get();
        return view('admin-views.pooja.panditpuja.details', compact('details'));
    }
    ...
    public function orders_generate_invoice($id)
    {
        ...
        $details = Service_order::where('id', $id)->where('indivisual', 1)->with(['customers', 'services', 'leads', 'selectedPackage', 'packages', 'payments', 'product_leads.productsData'])->first();
        // dd($details);
        $mpdf_view = PdfView::make('admin-views.pooja.panditpuja.invoice', compact('details', 'companyPhone', 'companyEmail', 'companyName', 'companyWebLogo', 'companyAddress', 'companygst', 'companypan'));
    ...
    //     $orders = Service_order::where('type', 'panditpooja')
    ...
    // {
    //     $processedCustomers = [];
    //     $orders = Service_order::where('indivisual',1)->where('type', 'panditpooja')->where('is_block', '!=', 9)->where('id', $id)->where('payment_status', 1)->where('status', 0)
    //     ->with(['services', 'customers', 'customer','payments'])->get();
    //     $order = $orders->first(); 
    //     // If order not found → stop & return error
    //     if (!$order) {
    ...
    //         $certificate = Image::make(public_path('assets/back-end/img/certificate/pooja/format/certificate-formato.png'));
    //         $certificate->text(ucwords($order->customers->f_name . ' ' . $order->customers->l_name), 950, 630, function ($font) {
    ...
    //             $font->valign('top');
    //         });
    //         $serviceName = wordwrap($order->services->name, 65, "\n", false);
    ...
    //         $certificate->text(date('d/m/Y', strtotime($order->booking_date)), 830, 994, function ($font) {
    //             $font->file(public_path('fonts/NotoSans-Regular.ttf'));
    //             $font->size(40);
    //             $font->color('#ffffff');
    //             $font->align('center');
    ...
    //         $certificatePath = 'assets/back-end/img/certificate/pooja/' . $order['order_id'] . '.jpg';
    //         $certificate->save(public_path($certificatePath));
    //         // Service_order::where('order_id', $order['order_id'])->update(['pooja_certificate' => $order['order_id'] . '.jpg']);
    //         $astrologer = Astrologer::where('id', $order->pandit_assign)->first();
    //         if ($astrologer) {
    //             foreach (json_decode($astrologer['is_pandit_pooja_commission']) as $key => $value) {
    //                 if ($key == $order['services']['id']) {
    //                     $commission = $value;
    //                 }
    //             }
    //         }
    //         if (!$order->pandit_assign) {
    //             Toastr::error(translate('Please_selected_the_Pandit_ji'));
    //             return back();
    //         }
    //         $transaction = new ServiceTransaction();
    //         $transaction->astro_id = $order['pandit_assign'];
    //         $transaction->type = 'panditpooja';
    //         $transaction->order_id = $order['order_id'];
    //         $transaction->txn_id = !empty($order['wallet_translation_id']) ? $order['wallet_translation_id'] : $order['payment_id'];
    ...
    //         $transaction->save();
    //         Devotee::where('service_order_id', $order->order_id)->update([
    //             'status' => 1,
    //         ]);

    //         PanditTransectionPooja::where('service_order_id', $order->order_id)->update([
    //             'status' => 1,
    //         ]);
    //         PoojaRecords::where('service_order_id', $order->order_id)->update([
    //             'status' => 1,
    //         ]);
    //         Service_order::where('order_id', $order['order_id'])->where('is_block', '!=', 9)->update([
    ...
    //         Prashad_deliverys::where('order_id', $order['order_id'])->where('service_id', $request->service_id)->update([
    ...
    //             'share_video'    => $order->pooja_video ?? 'mahakal.com',
    //             'customer_id'    => $order->customer_id,
    //             'puja_venue'     => $service->final_venue ?? 'N/A',
    //             'attachment'     => asset('public/' . $certificatePath),
    //             'type'           => 'text-with-media',
    //             'orderId'        => $order->order_id,
    //             'amount'         => webCurrencyConverter((float) ($order->pay_amount - ($order->coupon_amount ?? 0))),
    ...
    //         return redirect()->route('admin.pandit.orders.list', ['status' => 1]);
    //     }
    // }
    ...
    public function orders_status($id, Request $request)
    {
        ...
        if ($request->order_status == 1) {
            ...
            // Get astrologer info and commission
            $astrologer = Astrologer::where('id', $order->pandit_assign)->first();
             if ($astrologer) {
            // Use direct commission from column individual_commission
            $commission = $astrologer->individual_commission ?? 0;
            } else {
                $commission = 0;
            }
            ...
            $adminAmount  = (int) floor(($amount * $commissionPercent) / 100);
            $panditAmount = (int) ($amount - $adminAmount);
            ...
            $transaction->commission = $commissionPercent;
            $transaction->admin_amount = $adminAmount;
            $transaction->pandit_amount = $panditAmount;  
            $transaction->tax = $tax['online_pooja'];
            $transaction->save();
        ...
        // =====================================================
        if ($request->order_status == 1 && $order->pandit_assign) {

           $panditId = $order->pandit_assign;
           $panditAmount = $panditAmount ?? 0;

            if ($panditAmount > 0) {

             // Last balance
            $lastBalance = \App\Models\INPanditWallet::where('pandit_id', $panditId)
               ->orderBy('id', 'desc')
               ->value('balance') ?? 0;

            \App\Models\INPanditWallet::create([
            'pandit_id'    => $panditId,
            'service_id'   => $order->service_id,
            'booking_date' => $order->booking_date,
            ...
            'credit'       => $panditAmount,
            'debit'        => 0,
            'balance'      => $lastBalance + $panditAmount,
    ...
    // Certificate Update The Code Pandit Puja
    public function updateCertificate(Request $request, $id)
    {
        // Always use first() instead of get()
        $order = Service_order::where('indivisual', 1)
            ->where('type', 'panditpooja')
            ->where('is_block', '!=', 9)
            ->where('id', $id)
            ->where('payment_status', 1)
            ->where('status', 1)
            ->with(['services', 'customers', 'customer', 'payments'])
            ->first();   // <--- FIXED
        ...
        // Certificate Path based on order_id
        $certificatePath = 'assets/back-end/img/certificate/pooja/' . $order->order_id . '.jpg';
        ...
        if ($request->hasFile('pooja_certificate')) {
            ...
            if ($order->pooja_certificate && file_exists(public_path($order->pooja_certificate))) {
                @unlink(public_path($order->pooja_certificate));
            }
            $file = $request->file('pooja_certificate');
            $file->move(public_path('assets/back-end/img/certificate/pooja/'), $order->order_id . '.jpg');

            $order->pooja_certificate = $certificatePath;
            $order->save();
        }
    ...
    {
        $orders = Service_order::where('type', 'panditpooja')->where('is_block', '!=', 9)->where('service_id', $request->service_id)->where('booking_date', $request->booking_date)->where('payment_status', 1)->with(['services', 'customers', 'payments', 'customer'])->get();
        if (!$orders) {
            ...
            if ($pooja->customers) {
                ...
                if ($userInfo && !empty($userInfo['email']) && filter_var($userInfo['email'], FILTER_VALIDATE_EMAIL)) {
                    ...
                    $bookingDetails = \App\Models\Service_order::where('service_id', ($pooja->service_id ?? ""))
                        ->where('type', 'panditpooja')->where('booking_date', ($pooja->booking_date ?? ""))
    ...
    {
        $orders = Service_order::where('status', 0)->where('type', 'panditpooja')->where('is_block', '!=', 9)->where('booking_date', $request->booking_date)->where('payment_status', 1)->where('service_id', $request->service_id)->with(['services', 'customers', 'customer'])->get();
        if ($orders->isEmpty()) {
    ...
    {
        // dd($request->all());
        $orders = Service_order::where('status', 0)->where('payment_status', 1)->where('is_block', '!=', 9)->where('type', 'panditpooja')->where('booking_date', $request->booking_date)->where('service_id', $request->service_id)->with(['services', 'customers', 'customer'])->get();
        if ($orders->isEmpty()) {
            Toastr::error(translate('No orders found for the given criteria.'));
            return back();
        }
    ...
    public function lead_list(Request $request)
    {
        if ($request->has('searchValue')) {
            $leads = Leads::where('person_name', 'like', '%' . $request->searchValue . '%')->where('status', 1)->where('type', 'panditpooja')->with('service')->orderBy('created_at', 'DESC')->paginate(10);
        } else {
            $leads = Leads::where('status', 1)->where('type', 'panditpooja')->with('service', 'followby')->orderBy('created_at', 'DESC')->paginate(10);
        }
        return view('admin-views.pooja.panditpuja.lead.list', compact('leads'));
    }
    ...
    {
        $orderId = $request->id;
        $order = Service_order::where('id', $orderId)->where('type', 'panditpooja')->where('is_block', '!=', 9)
    ...
    public function orders_by_pooja(Request $request)
    {
        ...
        $orders = Service_order::where('type', 'panditpooja')->where('status', 0)->where('is_block', '!=', 9)->where('is_completed', 0)->where('payment_status', 1)
            ...
            ->get();
        $users = User::all();
        return view('admin-views.pooja.panditpuja.orderbypooja', compact('orders', 'users'));
    }
    ...
    function all_single_order($service_id, $booking_date, $status)
    {
        ...
        $orders = Service_order::where('type', 'panditpooja')->where('service_id', $service_id)->where('booking_date', $booking_date)->where('is_block', '!=', 9)
            ->where('order_status', $status)->with(['services','leads'])->get();
        $pending = Service_order::where('type', 'panditpooja')->where('service_id', $service_id)->where('booking_date', $booking_date)->where('order_status', $status)->where('is_block', '!=', 9)->first();
        if (!$pending) {
            $pending = null;
        }
        // dd($orders);
        return view('admin-views.pooja.panditpuja.AllsingleOrder', compact('orders', 'service', 'pending'));
    }
    ...
    public function single_orders_details($booking_date, $service_id, $status)
    {
        ...
        return view('admin-views.pooja.panditpuja.SingleOrderdetails', compact('details', 'service', 'inHouseAstrologers', 'freelancerAstrologers'));
    }
    ...
    public function downloadMemberList($service_id, $booking_date, $status)
    {
        ...
        $mpdf_view = PdfView::make('admin-views.pooja.panditpuja.member-list', compact('orders', 'pooja_name', 'bookingDate', 'pooja_venue'));
    ...
    //     $users = User::all();
    //     // dd($orders);
    //     return view('admin-views.pooja.panditpuja.orderbycompleted', compact('orders', 'users'));
    // }
    ...
    public function completed_puja($booking_date, $service_id, $status)
    {
        ...
        return view('admin-views.pooja.panditpuja.complete-puja', compact(
            'orders',
            'details',
            'inHouseAstrologers',
    ...
    public function update_pooja_video(Request $request)
    {
        ...
        $orders = Service_order::where('order_status', $request->order_status)->where('is_block', '!=', 9)->where('type', 'panditpooja')->where('booking_date', $request->booking_date)->where('service_id', $request->service_id)->with(['services', 'customers', 'customer'])->get();
