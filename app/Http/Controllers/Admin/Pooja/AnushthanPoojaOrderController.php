...
class AnushthanPoojaOrderController extends Controller
{
    ...
    public function orders_list($status, Request $request)
    {
        ...
        Service_order::where('type', 'anushthan')->where('package_id', 7)->where('is_block', '!=', 9)->where('status', 0)->where('order_status', 0)
        ...
        if ($status == 'all') {
            $orders = Service_order::where('type', 'anushthan')->where('is_block', '!=', 9)->with('leads')->with('packages')->with('vippoojas')->with('customers')->with('pandit')->orderBy('created_at', 'DESC')->paginate(10);
        } else {
            $orders = Service_order::where('type', 'anushthan')->where('is_block', '!=', 9)->where('status', $status)->with('leads')->with('customers')->with('vippoojas')->with('pandit')->orderBy('created_at', 'DESC')->paginate(10);
        }
        $users = User::all();
    ...
    {
        $orders = Service_order::where('type', 'anushthan')->where('is_block', '!=', 9)->where('status', 0)->where('is_completed', 0)->where('package_id', 8)->where('payment_status', 1)
    ...
    {
        // dd($request->all());
        $orders = Service_order::where('id', $id)->where('type', 'anushthan')->where('is_block', '!=', 9)->where('service_id', $request->service_id)->where('payment_status', 1)->where('booking_date', $request->booking_date)->where('is_completed', 0)->where('package_id', $request->package_id)->with(['vippoojas', 'customers', 'payments'])->first();
        if (!$orders) {
        ...
        if ($request->order_status == 1) {
            ...
            $certificatePath = 'assets/back-end/img/certificate/anushthan/' . $orders['order_id'] . '.jpg';
                ...
                if ($panditId) {
                    ...
                    if ($slab) {
                        ...
                        if ($creditAmount >= 0) {
                            ...
                            // ✅ IMPORTANT FIX: entry_type added
    ...
    {
        $pandit = Service_order::where('type', 'anushthan')->where('is_block', '!=', 9)->where('service_id', $request->service_id)->where('booking_date', $request->booking_date)->update(['pandit_assign' => $request->pandit_id]);
        // dd($pandit);
        if ($pandit) {
    ...
    {
        $orders = Service_order::where('type', 'anushthan')->where('is_block', '!=', 9)->where('service_id', $request->service_id)->where('booking_date', $request->booking_date)->where('package_id', 7)->with(['vippoojas', 'customers', 'payments'])->get();
        if (!$orders) {
            Toastr::error(translate('order_not_found'));
            return back();
        }
    ...
    public function status_time(Request $request)
    {
        ...
        $orders = Service_order::where('type', 'anushthan')->where('is_block', '!=', 9)->where('status', 0)->where('package_id', $request->package_id)->where('booking_date', $request->booking_date)->where('service_id', $request->service_id)->with(['vippoojas', 'customers'])->get();
        foreach ($orders as $pooja) {
    ...
    {
        $orderId = $request->id;
        $order = Service_order::where('id', $orderId)->where('type', 'anushthan')->where('is_block', '!=', 9)
    ...
    function all_single_order($service_id, $booking_date, $status)
    {
        ...
        $orders = Service_order::where('type', 'anushthan')->where('is_block', '!=', 9)->where('service_id', $service_id)->where('booking_date', $booking_date)->where('order_status', $status)->with(['vippoojas'])->get();
        // dd($orders);
        $pending = Service_order::where('type', 'anushthan')->where('is_block', '!=', 9)->where('service_id', $service_id)->where('booking_date', $booking_date)->where('order_status', $status)->first();
        if (!$pending) {
            $pending = null;
        }
        // dd($pending);
    ...
    public function instance_orders_details($booking_date, $service_id, $status)
    {
        ...
        $query = Service_order::where('type', 'anushthan')->where('is_block', '!=', 9)->where('service_id', $service_id)->where('booking_date', $booking_date)->where('status', $status)->where('package_id', 8)->with(['vippoojas', 'packages', 'pandit'])
            ->selectRaw('service_id, COUNT(*) as total_orders, pandit_assign, booking_date, COUNT(booking_date) as booking_count, SUM(pay_amount) as total_amount, 
                GROUP_CONCAT(COALESCE(members, "") SEPARATOR "|") as members, 
                GROUP_CONCAT(COALESCE(gotra, "") SEPARATOR "|") as gotra, 
                GROUP_CONCAT(COALESCE(leads_id, "") SEPARATOR ",") as leads, 
    ...
    {
        $orders = Service_order::where('type', 'anushthan')->where('is_block', '!=', 9)->where('service_id', $service_id)->where('booking_date', $booking_date)->where('order_status', $status)->get();
    ...
    public function update_anushthan_certificate(Request $request)
    {
        $request->validate([
            'order_id' => 'required',
        ...
        $path = 'assets/back-end/img/certificate/anushthan/';
        ...
        if ($request->hasFile('pooja_certificate')) {
            if (!empty($order->pooja_certificate) &&
                File::exists(public_path($order->pooja_certificate))) {
        ...
        Toastr::success('Anushthan certificate updated successfully');
        return back();
    }
}