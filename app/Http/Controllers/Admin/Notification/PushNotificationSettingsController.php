...
class PushNotificationSettingsController extends BaseController
{
    ...
    public function getView(): View
    {
        ...
        $pujaMessages = $this->getPushNotificationMessageData(userType: 'puja');
        $counsellingMessages = $this->getPushNotificationMessageData(userType: 'counselling');
        $offlinepujagMessages = $this->getPushNotificationMessageData(userType: 'offlinepuja');
        $language = $this->businessSettingRepo->getFirstWhere(params: ['type' => 'pnc_language']);
        return view(PushNotification::INDEX[VIEW], compact('customerMessages', 'vendorMessages', 'deliveryManMessages', 'pujaMessages', 'counsellingMessages', 'offlinepujagMessages', 'language'));
    }
