...
class ConfigController extends Controller
{
    public function configuration(Request $request)
    {
        ...
        if (!empty($request->type) && ($request->type == 'seller' || $request->type == 'tour' || $request->type == 'event' || $request->type == 'trustees')) {
            $privacy_policy_comm = Helpers::get_business_settings($request->type . '_privacy_policy');
            $faq_comm = \App\Models\FAQ::whereHas('Category', function ($query) use ($request) {
                if ($request->type == 'trustees') {
                    $query->where('name', 'trustee_vendor');
                } else {
                    $query->where('name', $request->type . '_vendor');
                }
            })->orderBy('id', 'desc')->with('Category')->get();
            $terms_ands_conditions_comm = Helpers::get_business_settings($request->type . '_terms_condition');
        } else {
            $privacy_policy_comm = Helpers::get_business_settings('privacy_policy');
            $faq_comm = HelpTopic::all();
            $terms_ands_conditions_comm = Helpers::get_business_settings('terms_condition');
        }
        ...
        return response()->json([
            ...
            'privacy_policy' => $privacy_policy_comm,
            'faq' => $faq_comm,
            'terms_&_conditions' => $terms_ands_conditions_comm,
