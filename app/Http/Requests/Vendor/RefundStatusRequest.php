...
class RefundStatusRequest extends FormRequest
{
     ...
     */
        public function rules(): array
        {
            return [
                'id' => 'required',
                'refund_status' => 'required|in:pending,approved,rejected,refunded,received',
                'approved_note' => $this->input('refund_status') == 'approved' ? 'required' : '',
                'rejected_note' => $this->input('refund_status') == 'rejected' ? 'required': '',
                'received_note' => $this->input('refund_status') == 'received' ? 'required': '',
            ];
        }
        public function messages(): array
        {
            return [
                ...
                'received_note.required' => translate('The_received_note_field_is_required'),
            ];
        }
