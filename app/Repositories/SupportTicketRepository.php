        ...
        $query = $this->supportTicket
            ...
            ->when($searchValue, function ($query) use($searchValue){
                return $query->Where('subject', 'like', "%{$searchValue}%")
                    ...
                    ->orWhere('status', 'like', "%{$searchValue}%")->orWhereHas('TicketType', function ($q) use ($searchValue) {
                        $q->where('name', 'like', "%$searchValue%");
                    })->orWhereHas('TicketIssue', function ($q) use ($searchValue) {
                        $q->where('issue_name', 'like', "%$searchValue%");
                    });
            })
            ...
            ->when(isset($filters['names']), function ($query) use ($filters) {
                return $query->orWhereHas('TicketType', function ($q) use ($filters) {
                    $q->where('name', $filters['names']);
                });
            })
