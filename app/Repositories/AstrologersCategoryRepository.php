...
class AstrologersCategoryRepository implements AstrologersCategoryRepositoryInterface {
    ...
    public function getListWhere(array $orderBy = [], ?string $searchValue = null, array $filters = [], array $relations = [], int|string $dataLimit = DEFAULT_DATA_LIMIT, ?int $offset = null): Collection|LengthAwarePaginator
    {
        $query = $this->categorydata->with($relations)
            ...
            ->when(isset($searchValue), function ($query) use ($searchValue) {
                $translation_ids = $this->translation->where('translationable_type', 'App\Models\Astrologer\AstrologerCategory')
                    ->where('key', 'name')
    ...
    public function delete(array $params): bool
    {
        ...
        $this->translation->where('translationable_type', 'App\Models\Astrologer\AstrologerCategory')->where('translationable_id', $params['id'])->delete();
