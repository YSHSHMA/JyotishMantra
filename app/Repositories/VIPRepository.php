...
class VIPRepository implements VIPRepositoryInterface
{
    ...
    public function addVIPTag(object $request, object $vip): void
    {
        $tagIds = [];
        ...
        $vip->tags()->sync($tagIds);
    }
    public function add(array $data): string|object
    {
        return $this->vip->create($data);
    }

    public function getVIPFirstWhere(array $params, array $relations = []): ?Model
    {
        return $this->vip->where($params)->with($relations)->first();
    }

    public function getFirstWhere(array $params, array $relations = []): ?Model
    {
        return $this->vip->withoutGlobalScope('translate')->with($relations)->where($params)->first();
    }
    public function getFirstWhereActive(array $params, array $relations = []): ?Model
    {
        return $this->vip->active()->where($params)->with($relations)->first();
    }

    public function getFirstWhereWithoutGlobalScope(array $params, array $relations = []): ?Model
    {
        return $this->vip->withoutGlobalScopes()->where($params)->with($relations)->first();
    }

    public function getList(array $orderBy = [], array $relations = [], int|string $dataLimit = DEFAULT_DATA_LIMIT, int $offset = null): Collection|LengthAwarePaginator
    {
        $query = $this->vip->with($relations)
    ...
    {
        return $this->vip->where('id', $id)->update($data);
    }

    public function delete(array $params): bool
    {
        $this->vip->where($params)->delete();
        return true;
    }
