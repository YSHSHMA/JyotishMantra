...
$("#choice_attributes").on("change", function () {
  ...
  if (
    (colors.length === 0 && choiceAttributes.length === 0) ||
    (!$("#product-color-switcher").prop("checked") &&
  ...
  $.each($("#choice_attributes option:not(:selected)"), function () {
    ...
    let className = `${selectedText.replace(
      /[^a-zA-Z0-9]/g,
      ""
    )}${selectedValue.replace(/[^a-zA-Z0-9]/g, "")}`;
...
function addMoreCustomerChoiceOption(index, name) {
  ...
  if (
    $("#customer_choice_options").find(`[name="choice_no[]"][value="${index}"]`)
      .length === 0
  ) {
    let genHtml =
      `<div class="col-md-6 ${name.replace(/[^a-zA-Z0-9]/g, "")}${index.replace(
        /[^a-zA-Z0-9]/g,
        ""
                  ...
                  <div class="">
                      <input type="text" class="form-control option_value_new${index}" name="choice_options_${index}[]" placeholder="` +
