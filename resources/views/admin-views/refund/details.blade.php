                ...
                <div class="card h-100 refund-details-card">
                    <div class="card-body">
                        ...
                        <ul class="dm-info p-0 m-0">
                            ...
                            <li class="align-items-center">
                                <span class="left">{{translate('rma_no')}} </span> <span>:</span> <span class="right">
                                    {{ $refund['rma_no'] }}</span>
                            </li>
                            <li class="align-items-center">
                                <span class="left">{{translate('Return AWB')}} </span> <span>:</span> <span class="right">
                                    {{ $order->third_party_delivery_tracking_id }}</span>
                            </li>
                            @endif
                            <li class="align-items-center">
                                <span class="left">{{translate('order_details')}}({{ $refund['order_id'] }}) </span> <span>:</span> <span class="right"><a class="badge py-2 badge-soft-primary border border-primary px-2" href="{{route('admin.orders.details',['id'=>$order->id])}}">{{translate('view_details')}}</a></span>
                            </li>
                        </ul>
                    </div>
                </div>
                        ...
                        <div class="gap-3 mb-4 d-flex justify-content-between flex-wrap align-items-center">
                            ...
                            <div class="d-flex flex-wrap gap-3">
                                @if($refund['status'] != 'refunded')
                                    ...
                                    @endif
                                    @if($refund['status'] != 'approved' && $refund['status'] != 'received')
                    ...
                    <div class="card-body">
                        ...
                        <p>Customer Name:<b>{{ $order->shipping_address_data->contact_person_name }}</b></p>
                            <p>Customer Address:<b>{{ $order->shipping_address_data->address }} 
                                {{ $order->shipping_address_data->city }} {{ $order->shipping_address_data->zip }}</b></p>
                                <p>product Reason & Image :
                            {{$refund->reason_data}}( {{$refund->reason_name}} )</p>
                    ...
                    <div class="card h-100 refund-details-card--2">
                        <div class="card-body">
                            ...
                            <div class="key-val-list d-flex flex-column gap-2 min-width--60px">
                                ...
                                <div class="key-val-list-item d-flex gap-3">
                                    <span class="text-capitalize">{{translate('Warehouse_id')}}</span>:
                                    <span>{{$order->seller?->warehouse_id ?? translate('no_data_found')}}</span>
                                </div>
                            </div>
                        </div>
                    </div>
