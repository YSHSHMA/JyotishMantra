            ...
            <div class="col-md-12">
                <div class="card mb-4 shadow-sm rounded">
                    <div class="card-header d-flex justify-content-between align-items-center bg-light border-bottom">
                        ...
                        <!-- Right side (button) -->
                        @if (Helpers::modules_permission_check('All Service', 'Lead', 'add'))
                        ...
                        <div class="row">
                            ...
                            <div class="col-12 col-md-6 col-lg-4 mb-3">
                                ...
                                <select class="serviceType form-control">
                                    ...
                                    <option value="panditcounselling">Pandit Counselling</option>
                                    <option value="panditpooja">Pandit Pujag</option>
                                </select>
                            </div>
...
@push('script')
    ...
    <script>
        const leadDeleteBaseUrl = "{{ route('admin.leads.lead-delete', ':leadno') }}";
        const whatsappBaseUrl = "{{ route('admin.leads.send-whatsapp-leads', ':leadno') }}";
        const whatsappUpdateBaseUrl = "{{ route('admin.whatsapp.pooja-template') }}";
    </script>
                ...
                success: function(response) {
                    ...
                    if (response.length != 0) {
                        $.each(response, function(key, value) {
                            row +=
                                `<tr> <td>${key+1}</td> <td>${value.type}</td> <td>${value.follow_by}</td> <td>${new Date(value.last_date).toLocaleDateString('en-GB')}</td> <td>${value.message}</td> <td>${new Date(value.next_date).toLocaleDateString('en-GB')}</td> </tr>`;
                        });
                    } else {
