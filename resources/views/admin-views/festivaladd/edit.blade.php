...
@section('title', translate('rashi_Update'))
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-body text-start">
                        <form action="{{ route('admin.rashi.update', [$rashi['id']]) }}" method="post"
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    @foreach($language as $lang)
                                            <?php
                                            if (count($rashi['translations'])) {
                                                $translate = [];
                                                foreach ($rashi['translations'] as $translations) {
                                        ...
                                        <div class="form-group {{$lang != $defaultLanguage ? 'd-none':''}} form-system-language-form"
                                             id="{{$lang}}-form">
                                            <label class="title-color" for="name">{{ translate('rashi_Name') }}
                                                ({{ strtoupper($lang) }})</label>
                                            <input type="text" name="name[]"
                                                   value="{{$lang == $defaultLanguage ? $rashi['name']:($translate[$lang]['name']??'') }}"
