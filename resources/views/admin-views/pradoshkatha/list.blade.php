...
@section('title', translate('pradosh_katha_List'))
...
@section('content')
    <div class="content container-fluid">
        <div class="mb-3">
            <h2 class="h1 mb-0 d-flex gap-2">
                ...
                {{ translate('pradoshkatha_List') }}
                <span class="badge badge-soft-dark radius-50 fz-14">{{ $pradoshkathas->total() }}</span>
            </h2>
        </div>
        <div class="row mt-20">
            <div class="col-md-12">
                        ...
                        <div class="row g-2 flex-grow-1">
                            ...
                            <div class="col-sm-4 col-md-6 col-lg-8 d-flex justify-content-end">
                                ...
                                <ul class="dropdown-menu">
                                    <li>
                                        <a class="dropdown-item" href="{{ route('admin.pradoshkatha.export', ['searchValue'=>request('searchValue')]) }}">
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover table-borderless table-thead-bordered table-nowrap table-align-middle card-table w-100 text-start">
                                ...
                                <tr>
                                    ...
                                    @if (Helpers::modules_permission_check('Pradosh Katha', 'Pradosh Katha', 'edit') || Helpers::modules_permission_check('Pradosh Katha', 'Pradosh Katha', 'delete'))
                                    <th class="text-center"> {{ translate('action') }}</th>
                                    @endif
                                </tr>
                                </thead>
                                <tbody>
                                @foreach($pradoshkathas as $key => $pradoshkatha)
                                    <tr>
                                        <td>{{ $pradoshkathas->firstItem()+$key }}</td>
                                        
                                        <td>{{$pradoshkatha->title}}</td>
                                        
                                        <td class="overflow-hidden max-width-100px">
                                            <span data-toggle="tooltip" data-placement="right" title="{{$pradoshkatha['description']}}">
                                                 {!!Str::limit($pradoshkatha['description'],20) !!}
                                            </span>
                                        </td>
                                        @if (Helpers::modules_permission_check('Pradosh Katha', 'Pradosh Katha', 'edit') || Helpers::modules_permission_check('Pradosh Katha', 'Pradosh Katha', 'delete'))
                                        <td>
                                            <div class="d-flex justify-content-center gap-2">
                                                @if (Helpers::modules_permission_check('Pradosh Katha', 'Pradosh Katha', 'edit'))
                                                <a class="btn btn-outline-info btn-sm square-btn" title="{{ translate('edit') }}"
                                                    href="{{ route('admin.pradoshkatha.update', [$pradoshkatha['id']]) }}">
                                                    <i class="tio-edit"></i>
                                                </a>
                                                @endif

                                                @if (Helpers::modules_permission_check('Pradosh Katha', 'Pradosh Katha', 'delete'))
                                                <a class="btn btn-outline-danger btn-sm delete delete-data" href="javascript:"
                                                data-id="pradoshkatha-{{$pradoshkatha['id']}}"
    ...
    <span id="get-pradoshkathas" data-pradoshkathas="{{ json_encode($pradoshkathas) }}"></span>
