                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <select {{ $lang == $defaultLanguage ? 'required' : '' }} name="tour_type" id="{{ $lang }}tour_type" class="form-control @error('tour_type') is-invalid @enderror tour_types" onchange="$('.tour_types').val(this.value)">
                                    <option value="cities_tour" {{ ((old('tour_type') == 'cities_tour' )?'selected':'' ) }}>Cities Tour</option>
                                    <option value="special_tour" {{ ((old('tour_type') == 'special_tour' )?'selected':'' ) }}>special Tour</option>
                                </select>
                            </div>
                        </div>
                        <div class="col-md-4">
                ...
                <div class="row">
                        ...
                        <div class="row mt-2">
                            <div class='col-3 p-0 pr-1'>
                                <select class="form-control point_trigger1{{ $index }}" name="cab_id[{{ $index }}]" onchange="select_value(this)" data-point='point_trigger1{{ $index }}'>
                                    ...
                                    <option value="{{ $cabs['id'] }}" {{ $cabs['id'] == $oldCabId ? 'selected' : '' }}>{{ $cabs['name'] }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class='col-3 p-0 pr-1'>
                                <select class="form-control select2-multiple point_trigger2{{ $index }}" multiple name="package_id[{{ $index }}][]" onchange="select_value(this)" data-point='point_trigger2{{ $index }}'>
                                    @foreach($package_list as $packval)
                                    <option value="{{ $packval['id'] }}" {{ in_array($packval['id'], $oldPackages[$index] ?? []) ? 'selected' : '' }}>{{ $packval['name'] }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class='col-3 p-0 pr-1'>
                                <input type='text' class="form-control people_no point_trigger3{{ $index }}" name="people[{{ $index }}]" value="{{ $oldPeople[$index] ?? '' }}" onkeyup="select_value(this)" data-point='point_trigger3{{ $index }}' placeholder="{{ translate('enter_max_people') }}">
                            </div>
                            <div class="col-2 p-0 pr-1">
                                <input type='text' class="form-control price_no point_trigger4{{ $index }}" name="price[{{ $index }}]" value="{{ $oldPrices[$index] ?? '' }}" onkeyup="select_value(this)" data-point='point_trigger4{{ $index }}' placeholder="{{ translate('enter_Price') }}">
                            </div>
                            @if($loop->index == 0)
                            <div class="col-1 p-0">
                        ...
                        <div class="row mt-2">
                            <div class='col-3 p-0 pr-1'>
                                <select class="form-control point_trigger1" name="cab_id[0]" onchange="select_value(this)" data-point='point_trigger1'>
                                    ...
                                    @if($cab_list)
                                    @foreach($cab_list as $cabs)
                                    <option value="{{ $cabs['id']}}" {{ (collect(old('cab_id'))->contains($cabs['id'])) ? 'selected' : '' }}>{{ $cabs['name']}}</option>
                                    @endforeach
                                    @endif
                                </select>
                            </div>
                            ...
                            <div class='col-3 p-0 pr-1'>
                                <select class="form-control select2-multiple point_trigger2" name="package_id[0][]" multiple onchange="select_value(this)" data-point='point_trigger2'>
                                    @if($package_list)
                                    @foreach($package_list as $packval)
                                    <option value="{{ $packval['id']}}" {{ (collect(old('package_id'))->contains($packval['id'])) ? 'selected' : '' }}>{{ $packval['name']}}</option>
                                    @endforeach
                                    @endif
                                </select>
                            </div>
                            <div class='col-3 p-0 pr-1'>
                                <input type='text' class="form-control people_no point_trigger3" name="people[0]" onkeyup="select_value(this)" data-point='point_trigger3' placeholder="{{ translate('enter_max_people') }}">
                            </div>
                            <div class="col-2 p-0 pr-1">
                                <input type='text' class="form-control price_no point_trigger4" name="price[0]" onkeyup="select_value(this)" data-point='point_trigger4' placeholder="{{ translate('enter_Price') }}">
                            </div>
                            <div class="col-1 p-0">
                                <a class='btn btn--primary btn-sm p-1 mt-2' onclick="add_new_html()"><i class='tio-add'></i></a>
                            </div>
                    ...
                    <div class='col-md-3 form-group'>
                        ...
                        <select class="form-control" name='use_date' onchange="((this.value == 0)?$('.use_interested_and_not').addClass('d-none'):$('.use_interested_and_not').removeClass('d-none'))">
                            <option value="0" {{((old('use_date') == 0)?'selected':'' )}}>Not use Date</option>
                            <option value="1" {{((old('use_date') == 1)?'selected':'' )}}>use Date</option>
                        </select>
                    </div>
                    <div class="col-md-3 form-group  {{((old('use_date') == 0)?'d-none':'' )}} use_interested_and_not">
    ...
    function add_new_html() {
        var newRow = `
                ...
                <div class='col-3 p-0 pr-1'>
                    ...
                    <div class="col-2 p-0 pr-1">
                        <input type='text' class="form-control price_no point_trigger4${pointCounter}" name="price[${pointCounter}]" onkeyup="select_value(this)" data-point='point_trigger4${pointCounter}' placeholder="Enter Price">
                    </div>
                    <div class="col-1 p-0">
                        <a class='btn btn-danger btn-sm p-1 mt-2' onclick="remove_html(this)"><i class='tio-remove'></i></a>
                    </div>
