                                ...
                                <div class="{{ $lang != $defaultLanguage ? 'd-none' : '' }} form-system-language-form" id="{{ $lang }}-form">
                                    <div class="row mt-4">
                                        ...
                                        <div class='col-md-4 form-group'>
                                            <label class="title-color" for="Language">{{ translate('percentage_off') }}</label>
                                            <input type="text" class="form-control percentage_off_input" name="percentage_off" value="{{ old('percentage_off') }}" oninput="this.value=this.value.replace(/\D/g,'').slice(0,2);$('.percentage_off_input').val(this.value)" required placeholder="Please Enter Off Percentage">
                                        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function saveStepData(step) {
            ...
            fetch('{{ route("admin.tour_visits.insert-tour") }}', {
                    method: 'POST',
                    body: formData,
                    headers: {
...
<script type="text/javascript">
    ...
    function add_new_cab_html() {
        ...
        <div class="row mt-2">
            ...
            <div class="col-4 p-0 pr-1">
                <input type='text' class="form-control point_basic_trigger4${pointCounter_cab}" name="basicprice[${pointCounter_cab}]" value="" onkeyup="select_value(this);this.value = this.value.replace(/[^0-9]/g, '' );$('.point_trigger4${pointCounter_cab}').val(Number($(this).val()))" data-point='point_basic_trigger4${pointCounter_cab}' placeholder="{{ translate('enter_basic_Price') }}">
            </div>
            <div class="col-2 p-0 pr-1">
            ...
            <div class="col-2 p-0">
                <a class='btn btn-danger btn-sm p-1 mt-2' onclick="remove_html(this)"><i class='tio-remove'></i></a>`;
        let classadd = 'd-none';
        if ("{{ old('use_date',0) }}" == 3 || $('select[name="use_date"]').val() == 3) {
            classadd = ''
        }
        newRow += ` <a class='btn btn--primary btn-sm p-1 mt-2 cab-ex-distance-charge${pointCounter_cab} ${classadd} specialTourwithoutdate' onclick="cab_ex_distance_model('${pointCounter_cab}')"><i class="tio-bonnet_open"> bonnet_open </i></a>
...
<script>
    ...
    //     document.querySelectorAll("select[name^='food_package_id']").forEach(select => {
    //         let currentValue = select.value;
    //         let isIncluded = select.closest('.food-row')?.querySelector('.include-item')?.checked;
    ...
    //     document.querySelectorAll("select[name^='hotal_package_id']").forEach(select => {
    //         let currentValue = select.value;
    //         let isIncluded = select.closest('.hotel-row')?.querySelector('.include-item')?.checked;

    //         select.querySelectorAll("option").forEach(option => {
    //             if (
    //                 option.value &&
    //                 selectedValues_array.includes(option.value) &&
    //                 option.value !== currentValue &&
    //                 !isIncluded // ✅ अगर include-item checked है तो disable मत करो
    //             ) {
    //                 option.disabled = true;
    //             } else {
    //                 option.disabled = false;
