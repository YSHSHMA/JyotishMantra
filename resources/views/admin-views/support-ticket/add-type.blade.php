        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-body text-start">
                    <form action="{{ route('admin.support-ticket.type-store')}}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="row">
                            <div class="col-md-12">
                                <div>
                                    <div class="form-group form-system-language-form">
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="name" class="form-control" placeholder="{{ translate('ticket_type_name') }}" required>
                                        </div>
                                    </div>
                                </div>
                            </div>
            ...
            <div class="card">
                    ...
                    <div class="d-flex flex-wrap justify-content-between gap-3 align-items-center">
                        <div class="">
                            <h5 class="text-capitalize d-flex gap-1">
                                {{ translate('ticket_type_list') }}
                                <span class="badge badge-soft-dark radius-50 fz-12">{{ $TypeList->count() }}</span>
                            </h5>
                        </div>
                ...
                <div class="table-responsive">
                    <table
                        ...
                        <tbody>
                            @foreach ($TypeList as $key => $package)
                            <tr>
                                <td class="text-center">{{ $key + 1 }}</td>
                                <td class="text-center">{{ $package['name'] }}</td>
                                <td class="text-center">
                                <form action="{{route('admin.support-ticket.status-update') }}" method="post" id="brand-status{{$package['id']}}-form">
                                            @csrf
                                    ...
                                    <div class="d-flex justify-content-center gap-10">
                                        <div class="d-flex justify-content-center gap-2">
                                            <a class="btn btn-outline-info btn-sm square-btn"
                                                ...
                                                href="{{ route('admin.support-ticket.type-update', [$package['id']]) }}">
                                                <i class="tio-edit"></i>
                                            </a>
                                        ...
                                        <form action="{{ route('admin.support-ticket.delete-type', [$package['id']]) }}"
