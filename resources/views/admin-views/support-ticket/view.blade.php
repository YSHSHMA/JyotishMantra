...
<div class="content container-fluid">
    <div class="row">
        ...
        <div class="col-md-4">
            <a href="{{ route('admin.support-ticket.type-add') }}" class="btn btn-primary">Add Type +</a>&nbsp;&nbsp;
            <a href="{{ route('admin.support-ticket.issue-add') }}" class="btn btn-primary">Add Issue +</a>
        </div>
    </div>
    <div class="row">
        <div class="col-sm-6 col-lg-3 col-md-3  mt-2">
            <a class="order-stats order-stats_confirmed" href="{{ route('admin.support-ticket.view')}}">
                ...
                <span class="order-stats__title"> {{ \App\Models\SupportTicket::whereNotNull('ticket_type_id')->count()}} </span>
            </a>
        </div>
        @php
        ...
        <div class="col-sm-6 col-lg-3 col-md-3  mt-2">
            <a class="order-stats order-stats_confirmed" href="{{ route('admin.support-ticket.view',['names'=>($v_type['name']??'')])}}">
                <div class="order-stats__content">
                    <i class="tio-all_done">all_done</i>
                    <h6 class="order-stats__subtitle">{{ ucwords($v_type['name']??'')}}</h6>
                </div>
                <span class="order-stats__title">{{ \App\Models\SupportTicket::where('ticket_type_id',$v_type['id'])->count()}} </span>
            </a>
        </div>
        @endforeach
        @endif
                        ...
                        <div class="">
                            <div class="d-flex flex-wrap flex-sm-nowrap gap-3 justify-content-end">
                                ...
                                <select class="form-control border-color-c1 w-160 filter-tickets" data-value="priority">
                                    ...
                                    <option value="low" {{ $priority == 'low' ? 'selected' : '' }}>{{ translate('low') }}
                                    </option>
                    ...
                    <div class="card">
                        <div
                            ...
                            <div class="media gap-3">
                                <?php
                                $getCountunread = \App\Models\SupportTicketConv::where('support_ticket_id', $ticket->id)->where('read_admin_status', 0)->count();
                                ?>
                                @if($ticket->customer)
                                ...
                                <div class="media-body">
                                    ...
                                    <div class="d-flex flex-wrap gap-2 align-items-center">
                                        ...
                                        <h6 class="mb-0">{{ \App\Models\SupportIssue::where('id',($ticket->ticket_issue_id??''))->first()['issue_name']??"" }}
                                        </h6>
                                    </div>
