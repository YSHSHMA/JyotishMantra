        ...
        <div class="pt-4">
            <dl>
                <div class="d-flex gap-2 justify-content-between">
                    ...
                    <dd>{{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $cartItems['subtotal'] + $cartItems['discountOnProduct']), currencyCode: getCurrencyCode()) }}
                    </dd>
                </div>
                ...
                <div class="d-flex gap-2 justify-content-between">
                    ...
                    <dd>{{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: round($cartItems['discountOnProduct'], 2)), currencyCode: getCurrencyCode()) }}
                    </dd>
                </div>
                ...
                <div class="d-flex gap-2 justify-content-between">
                    ...
                    <dd>
                        <button id="extra_discount" class="btn btn-sm p-0" type="button" data-toggle="modal"
                ...
                <div class="d-flex gap-2 justify-content-between">
                    ...
                    <dd>{{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: round($cartItems['totalTax'], 2)), currencyCode: getCurrencyCode()) }}
                    </dd>
                </div>
            ...
            <div class="pt-4 mb-4">
                ...
                <ul class="list-unstyled option-buttons">
                    <li>
                        ...
                        <label for="cash"
                            class="btn btn--bordered btn--bordered-black px-4 mb-0">{{ translate('cash') }}</label>
                    </li>
                    <li>
                        <input type="radio" value="card" id="card" name="type" hidden>
                        <label for="card"
                            class="btn btn--bordered btn--bordered-black px-4 mb-0">{{ translate('card') }}</label>
                    </li>
                    ...
                    @if ($walletStatus)
                        <li class="{{ str_contains(session('current_user'), 'walking-customer') ? 'd-none' : '' }}">
                            ...
                            <label for="wallet"
                                class="btn btn--bordered btn--bordered-black px-4 mb-0">{{ translate('wallet') }}</label>
                        </li>
                    @endif
                </ul>
            </div>
        ...
        <div class="d-flex gap-2 justify-content-between align-items-center pt-3 bottom-sticky-buttons">
            ...
            @if ($cartItems['countItem'])
                @if (auth('admin')->user()->admin_role_id == 1 ||
                        Helpers::modules_permission_check('POS', 'Billing Section', 'cancel-order'))
                ...
                @if (auth('admin')->user()->admin_role_id == 1 ||
                        Helpers::modules_permission_check('POS', 'Billing Section', 'place-order'))
