...
<div class="d-flex gap-2 flex-wrap mb-3">
    <div class="dropdown flex-grow-1" id="dropdown-order-select">
        ...
        <div class="dropdown-menu px-2">
            @foreach ($cartNames as $cartName)
                <button class="dropdown-item border rounded mb-1 action-cart-change"
                    data-cart="{{ $cartName }}">{{ $cartName }}</button>
            @endforeach
    ...
    @if (auth('admin')->user()->admin_role_id == 1 ||
            Helpers::modules_permission_check('POS', 'Billing Section', 'new-order'))
