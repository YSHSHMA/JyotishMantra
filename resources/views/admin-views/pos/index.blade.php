...
@section('content')
        ...
        <div class="container-fluid">
            <div class="row">
                @if (auth('admin')->user()->admin_role_id == 1 ||
                        Helpers::modules_permission_check('POS', 'Product Section', 'view'))
                            ...
                            <div class="card-body pt-2" id="items">
                                <div class="pos-item-wrap">
                                    @foreach ($products as $product)
                                        @include('admin-views.pos.partials._single-product', [
                                            'product' => $product,
                                        ])
                                    @endforeach
                                </div>
                ...
                @if (auth('admin')->user()->admin_role_id == 1 ||
                        Helpers::modules_permission_check('POS', 'Billing Section', 'view'))
                        ...
                        <div class="card billing-section-wrap">
                            ...
                            <div class="card-body">
                                <div class="d-flex justify-content-end mb-3">
                                    ...
                                    @if (auth('admin')->user()->admin_role_id == 1 ||
                                            Helpers::modules_permission_check('POS', 'Billing Section', 'view-hold-order'))
                                        <button type="button"
                                ...
                                <div class="form-group d-flex gap-2">
                                    ...
                                    <select id='customer' name="customer_id" data-placeholder="Walking Customer"
                                        ...
                                        <option value="0" {{ $userId == 0 ? 'selected' : '' }}>
                                            {{ translate('walking_customer') }}</option>
                                        @foreach ($customers as $customer)
                                            <option value="{{ $customer->id }}"
                                                {{ $userId == $customer->id ? 'selected' : '' }}>{{ $customer->f_name }}
                                                {{ $customer->l_name }}
                                                ({{ $customer->phone }})
                                            </option>
                                        @endforeach
                                    ...
                                    @if (auth('admin')->user()->admin_role_id == 1 ||
                                            Helpers::modules_permission_check('POS', 'Billing Section', 'add-customer'))
    ...
    <span id="route-admin-pos-change-cart-editable"
        data-url="{{ route('admin.pos.change-cart') . '/?cart_id=:value' }}"></span>
    ...
    <span id="message-you-want-to-remove-all-items-from-cart"
        data-text="{{ translate('you_want_to_remove_all_items_from_cart') }}"></span>
    <span id="message-product-quantity-is-not-enough"
        data-text="{{ translate('product_quantity_is_not_enough') }}"></span>
    <span id="message-sorry-product-is-out-of-stock" data-text="{{ translate('sorry_product_is_out_of_stock') }}"></span>
    <span id="message-item-has-been-added-in-your-cart"
        data-text="{{ translate('item_has_been_added_in_your_cart') }}"></span>
    <span id="message-extra-discount-added-successfully"
        data-text="{{ translate('extra_discount_added_successfully') }}"></span>
    <span id="message-amount-can-not-be-negative-or-zero"
        data-text="{{ translate('amount_can_not_be_negative_or_zero') }}"></span>
    <span id="message-sorry-the-minimum-value-was-reached"
        data-text="{{ translate('sorry_the_minimum_value_was_reached') }}"></span>
    <span id="message-this-discount-is-not-applied-for-this-amount"
        data-text="{{ translate('this_discount_is_not_applied_for_this_amount') }}"></span>
    <span id="message-product-quantity-cannot-be-zero-in-cart"
        data-text="{{ translate('product_quantity_can_not_be_zero_or_less_than_zero_in_cart') }}"></span>
