    ...
    <div class="card card-chat justify-content-between">
        <div class="card-header flex-wrap gap-3">
            <div class="media d-flex gap-3">
                @if($supportTicket['Tour'] && $supportTicket['type'] == 'tour')
                <img class="rounded-circle avatar" src="{{ getValidImage(path: 'storage/app/public/tour_and_travels/doc/'.(isset($supportTicket['Tour']['image'])?$supportTicket['Tour']['image']:''), type: 'backend-basic') }}" alt="" />
                @elseif($supportTicket['seller'] && $supportTicket['type'] == 'seller')
                <img class="rounded-circle avatar" src="{{ getValidImage(path: 'storage/app/public/seller/'.(isset($supportTicket['seller']['image'])?$supportTicket['seller']['image']:''), type: 'backend-basic') }}" alt="" />
                @endif
                <div class="media-body">
                    @if($supportTicket['Tour'] && $supportTicket['type'] == 'tour')
                    <h6 class="font-size-md mb-1">{{isset($supportTicket['Tour']['owner_name'])?$supportTicket['Tour']['owner_name']:translate('not_found')}}</h6>
                    <div class="fz-12">{{isset($supportTicket['Tour']['phone_no'])?$supportTicket['Tour']['phone_no']:''}}</div>
                    @elseif($supportTicket['seller'] && $supportTicket['type'] == 'seller')
                    <h6 class="font-size-md mb-1">{{isset($supportTicket['seller']['f_name'])?$supportTicket['seller']['f_name']:translate('not_found')}}</h6>
                    <div class="fz-12">{{isset($supportTicket['seller']['phone'])?$supportTicket['seller']['phone']:''}}</div>
            ...
            <div class="d-flex align-items-center flex-wrap gap-3">
                ...
                <span class="font-weight-bold badge-soft-{{(($supportTicket['status']=='open')?'info':'danger')}} rounded px-2">{{translate($supportTicket['status'])}}</span>

            </div>
        </div>
        <div class="card-body p-3 overflow-y-auto height-220 flex-grow-1 msg_history d-flex flex-column-reverse" id="show_msg">
            @foreach($supportTicket['conversations']->reverse() as $key => $message)
            ...
            <div class="outgoing_msg">
                <div class="sent_msg">
                    <div class="received_withdraw_msg">
                        ...
                        @endif
                        @if($message['sender_type'] == 'user' || json_decode($message['attached']) !=null)
                        <span class="time_date fz-12 pt-2 d-flex justify-content-end">
                            {{$message->created_at->locale('en')->diffForHumans()}}
                        </span>
                        @endif
                    </div>
                </div>
            ...
            <div class="incoming_msg">
                <div class="received_msg p-2">
                    ...
                    @if($message['sender_type'] == 'admin' && json_decode($message['attached']) !=null)
        ...
        <div class="card-footer border-0">
            <div class="type_msg">
                <div class="input_msg_write">
                    <form class="needs-validation" href="{{route('admin.vendor-support-ticket.vendor-inbox.replay',['id'=>$supportTicket['id']])}}" method="post" enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" name="ticket_issue_id" value="{{$supportTicket['id']}}">
                        ...
                        <div class="position-relative d-flex align-items-center">
                            ...
                            <label class="py-0 px-3 d-flex align-items-center m-0 cursor-pointer position-absolute">
                                ...
                                <input type="file" name="attached[]" id="message-file-array" class="h-100 position-absolute w-100 " hidden multiple accept=".jpg, .png, .jpeg, .gif, .bmp, .tif, .tiff|image/*">
                            </label>
                            @endif
