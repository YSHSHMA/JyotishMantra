...
<div class="content container-fluid">
    ...
    <div class="row">
        <div class="col-sm-6 col-lg-3 col-md-3  mt-2">
            <a class="order-stats order-stats_confirmed" href="{{ route('admin.support-ticket.view')}}">
                ...
                <span class="order-stats__title"> {{ \App\Models\VendorSupportTicketConv::where('created_by','admin')->whereNotNull('ticket_id')->count()}} </span>
            </a>
        </div>
        @if($TypeList)
        ...
        <div class="col-sm-6 col-lg-3 col-md-3  mt-2">
            <a class="order-stats order-stats_confirmed">
                ...
                <span class="order-stats__title">{{ \App\Models\VendorSupportTicketConv::where('created_by','admin')->where('type',$v_type)->where('status','open')->count()}} /
                    <span class="order-stats__title text-danger">{{ \App\Models\VendorSupportTicketConv::where('created_by','admin')->where('type',$v_type)->where('status','close')->count()}} </span>
                </span>
            </a>
        </div>
        @endforeach
                        ...
                        <div class="card-body align-items-center d-flex flex-wrap justify-content-between gap-3 border-bottom">
                            ...
                            <div class="card-body align-items-center d-flex flex-wrap flex-md-nowrap justify-content-between gap-4">
                                ...
                                <div class="text-nowrap">
                                    <a class="btn btn--primary" href="{{ route('admin.vendor-support-ticket.admin-inbox.singleTicket', $ticket['id']) }}">
    ...
    <div class="modal fade add-issues" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{route('admin.vendor-support-ticket.admin-inbox.store-inbox')}}" method="post">
                    <div class="modal-body">
                        @csrf
                        <div class="row">
                            <div class="col-md-6">
                                <label for="">{{translate('select_type')}}</label>
                                <select name="type" class="form-control ticket_type_id">
                                    <option value="0" selected disabled>{{translate('select_type')}}</option>
                                    @if($TypeList)
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="ticket_id" class="form-control ticket_issue_id">
                                </select>
                            </div>
                            <div class="col-md-6">
                                <label for="">{{translate('select_vendor')}}</label>
                                <select name="vendor_id" class="form-control ticket_vendor_id">
                                </select>
                            </div>
    ...
    <script>
        const ticketIssues = JSON.parse(`{!! json_encode($support_list) !!}`);
        const ticketVendorList = JSON.parse(`{!! json_encode($vendor_list) !!}`);
        $('.ticket_type_id').on('change', function() {
        const ticketTypeId = $(this).val();
        
        const $ticketIssueDropdown = $('.ticket_issue_id');
        const $ticketVendors = $('.ticket_vendor_id');

        
        $ticketIssueDropdown.empty();
        $ticketVendors.empty();
        $ticketIssueDropdown.append('<option value="">Select Issue</option>');
        $ticketVendors.append('<option value="">Select Vendor</option>');
        if (ticketTypeId) {
            const filteredIssues = ticketIssues.filter(function(issue) {             
                return issue['type'] == ticketTypeId;
            });
            filteredIssues.forEach(function(issue) {
                $ticketIssueDropdown.append(`<option value = "${issue.id}" > ${issue.message}</option>`);
            });

            const filteredIssues1 = ticketVendorList.filter(function(issue) {             
                return issue['type'] == ticketTypeId;
            });
            filteredIssues1.forEach(function(issue) {
                $ticketVendors.append(`<option value = "${issue.relation_id}" > ${issue.f_name} ${issue.l_name}</option>`);
            });
