...
@section('title', translate('festival_List'))
@section('content')
<div class="content container-fluid">
   <div class="mb-3">
      <h2 class="h1 mb-0 d-flex gap-2">
         <img width="20" src="{{ dynamicAsset(path: 'public/assets/back-end/img/festival.png') }}" alt="">
         {{ translate('festival_List') }}
         <span class="badge badge-soft-dark radius-50 fz-14">{{ $fastFestival->total() }}</span>
      </h2>
   </div>
   <div class="row mt-20">
      <div class="col-md-12">
               ...
               <div class="row g-2 flex-grow-1">
                  ...
                  <div class="col-sm-4 col-md-6 col-lg-8 d-flex justify-content-end">
                     ...
                     <ul class="dropdown-menu">
                        <li>
                           <a class="dropdown-item" href="{{ route('admin.festival.export', ['searchValue'=>request('searchValue')]) }}">
                  ...
                  <table class="table table-hover table-borderless table-thead-bordered table-nowrap table-align-middle card-table w-100 text-start">
                     <thead class="thead-light thead-50 text-capitalize">
                        <tr>
                           ...
                           <th class="max-width-100px">{{ translate('hi_description') }}</th>
                           <th class="max-width-100px">{{ translate('image') }}</th>
                           <th class="text-center"> {{ translate('action') }}</th>
                        </tr>
                     </thead>
                     <tbody>
                        @foreach($fastFestival as $key => $festival)
                        <tr>
                           <td>{{ $fastFestival->firstItem()+$key }}</td>
                           <td>{{ $festival->event_name }}
                           </td>
                           <td>{{ $festival->event_type }}</td>
                           <td>{!! Str::limit($festival['en_description'],20) !!}</td>
                           <td>{!! Str::limit($festival['hi_description'],20) !!}</td>
                           <td>
...
<span id="get-fastFestival" data-fastFestival="{{ json_encode($fastFestival) }}"></span>
