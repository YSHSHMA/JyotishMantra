    ...
    <div class="content container-fluid">
        <div class="mb-3 d-flex justify-content-between align-items-center">
            <h2 class="h1 mb-0 text-capitalize d-flex align-items-center gap-2">
                <img width="20" src="{{ dynamicAsset(path: 'public/assets/back-end/img/withdraw-icon.png') }}"
                    alt="">
                {{ translate('withdraw') }}
            </h2>

            <a href="{{ route('admin.sellers.cash-withdraw-list') }}" class="btn btn-info" target="_blank"
                title="view Cash List">
                {{ translate('Cash Withdrawal') }}
            </a>
        </div>
        <div class="row">
            <div class="col-md-12">
                        ...
                        <div class="row gy-1 align-items-center justify-content-between">
                            ...
                            <div class="d-flex col-auto gap-3">
                                <select name="withdraw_status_filter" data-action="{{ url()->current() }}"
                                    ...
                                    <option value="all" {{ request('approved') == 'all' ? 'selected' : '' }}>
                                        {{ translate('all') }}</option>
                                    <option value="approved" {{ request('approved') == 'approved' ? 'selected' : '' }}>
                                        {{ translate('approved') }}</option>
                                    <option value="denied" {{ request('approved') == 'denied' ? 'selected' : '' }}>
                                        {{ translate('denied') }}</option>
                                    <option value="pending" {{ request('approved') == 'pending' ? 'selected' : '' }}>
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-right">
                                        <li>
                                            <a class="dropdown-item"
                                                href="{{ route('admin.sellers.withdraw-list-export-excel') }}?approved={{ request('approved') }}">
                                                <img width="14"
                            ...
                            <tbody>
                                @foreach ($withdrawRequests as $key => $withdrawRequest)
                                    <tr>
                                        ...
                                        <td>{{ setCurrencySymbol(currencyConverter($withdrawRequest['amount']), currencyCode: getCurrencyCode(type: 'default')) }}
                                        </td>

                                        <td>
                                            @if (isset($withdrawRequest->seller))
                                                <a href="{{ route('admin.sellers.view', $withdrawRequest->seller_id) }}"
                                                    class="title-color hover-c1">{{ $withdrawRequest->seller->f_name . ' ' . $withdrawRequest->seller->l_name }}</a>
                                            @else
                                        ...
                                        @if (Helpers::modules_permission_check('Vendor', 'Withdraws', 'detail'))
                                            <td>
                                                <div class="d-flex justify-content-center">
                                                    @if (isset($withdrawRequest->seller))
                                                        <a href="{{ route('admin.sellers.withdraw_view', ['withdraw_id' => $withdrawRequest['id'], 'seller_id' => $withdrawRequest->seller['id']]) }}"
