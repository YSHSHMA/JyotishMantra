...
@section('title', translate('Cash Withdraw information View'))
...
@section('content')
    <div class="content container-fluid">
        ...
        <div class="row">
                        ...
                        <div class="row">
                            <div class="col-md-4 mb-2 mb-md-0">
                                <div class="flex-start flex-wrap">
                                    ...
                                    <div class="mx-1"><h5>{{setCurrencySymbol(amount: usdToDefaultCurrency(amount: $cashwithdrawRequest->amount), currencyCode: getCurrencyCode(type: 'default'))}}</h5></div>
                                </div>
                                <div class="flex-start flex-wrap">
                                    <div><h5>{{translate('request_time').':'.' '}} </h5></div>
                                    <div class="mx-1">{{$cashwithdrawRequest->created_at}}</div>
                                </div>
                            </div>
                            <div class="col-md-4 mb-2 mb-md-0">
                                <div class="flex-start">
                                    <div class="title-color">{{translate('note').':'.' '}}</div>
                                    {{-- <div class="mx-1">{{$cashwithdrawRequest->transaction_note}}</div> --}}
                                </div>
                            </div>
                            <div class="col-md-4">
                                @if ($cashwithdrawRequest->status == 0)
                                ...
                                @else
                                    <div class="text-center float-end">
                                        <label class="badge badge-{{$cashwithdrawRequest?->status == 1 ? 'success' : 'danger'}} p-2 rounded-bottom">
                                            {{translate($cashwithdrawRequest?->status == 1 ? 'approved' : 'denied')}}
                                        </label>
                                    </div>
                                @endif
                            </div>
                        ...
                        <div class="card-body text-start">
                            ...
                            <div class="mt-2">
                                <div class="flex-start">
                                    ...
                                    <div class="mx-1"><h4>{{$cashwithdrawRequest?->seller->bank_name ?? translate('no_data_found')}}</h4></div>
                                </div>
                                <div class="flex-start">
                                    <div><h6>{{translate('branch').' '.':'.' '}}</h6></div>
                                    <div class="mx-1"><h6>{{$cashwithdrawRequest?->seller->branch ?? translate('no_data_found')}}</h6></div>
                                </div>
                                <div class="flex-start">
                                    <div><h6>{{translate('holder_name').' '.':'.' '}}</h6></div>
                                    <div class="mx-1"><h6>{{$cashwithdrawRequest?->seller->holder_name ?? translate('no_data_found')}}</h6></div>
                                </div>
                                <div class="flex-start">
                                    <div><h6>{{translate('account_no').' '.':'.' '}} </h6></div>
                                    <div class="mx-1"><h6>{{$cashwithdrawRequest?->seller->account_no ?? translate('no_data_found')}}</h6></div>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            {{-- @endif --}}
            @if($cashwithdrawRequest->seller->shop)
                <div class="col-md-4 mb-3">
                    <div class="card h-100">
                        <div class="card-body text-start">
                            ...
                            <div class="flex-start">
                                ...
                                <div class="mx-1"><h5>{{$cashwithdrawRequest->seller->shop->name}}</h5></div>
                            </div>
                            <div class="flex-start">
                                <div><h5>{{translate('phone').' '.':'.' '}}</h5></div>
                                <div class="mx-1"><h5>{{$cashwithdrawRequest->seller->shop->contact}}</h5></div>
                            </div>
                            <div class="flex-start">
                                <div><h5>{{translate('address').' '.':'.' '}}</h5></div>
                                <div class="mx-1"><h5>{{$cashwithdrawRequest->seller->shop->address}}</h5></div>
                            </div>
                ...
                <div class="card h-100">
                    <div class="card-body text-start">
                        ...
                        <div class="flex-start">
                            ...
                            <div class="mx-1"><h5>{{$cashwithdrawRequest->seller->f_name.' '.$cashwithdrawRequest->seller->l_name}}</h5></div>
                        </div>
                        <div class="flex-start">
                            <div><h5>{{translate('email').' '.':'}}</h5></div>
                            <div class="mx-1"><h5>{{$cashwithdrawRequest->seller->email}}</h5></div>
                        </div>
                        <div class="flex-start">
                            <div><h5>{{translate('phone').' '.':'}}</h5></div>
                            <div class="mx-1"><h5>{{$cashwithdrawRequest->seller->phone}}</h5></div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <form action="{{route('admin.sellers.cash-withdraw-status',[$cashwithdrawRequest['id']])}}" method="POST">
                            @csrf
