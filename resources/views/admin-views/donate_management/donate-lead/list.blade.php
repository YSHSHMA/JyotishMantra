...
@section('title', translate('Donate_lead'))
@push('css_or_js')
    ...
    <div class="content container-fluid">
        <div class="mb-3">
            <h2 class="h1 mb-0 d-flex gap-2">
                ...
                <span class="badge badge-soft-dark radius-50 fz-14">{{ $DonateLeads->total() }}</span>
            </h2>
        </div>
        <div class="row mt-20">
            <div class="col-md-12">
                            ...
                            <table
                                ...
                                <thead class="thead-light thead-50 text-capitalize">
                                    <tr>
                                        ...
                                        @if (Helpers::modules_permission_check('Donate', 'Lead', 'delete') ||
                                                Helpers::modules_permission_check('Donate', 'Lead', 'follow-up') ||
                                                Helpers::modules_permission_check('Donate', 'Lead', 'follow-up-history') ||
                                                Helpers::modules_permission_check('Donate', 'Lead', 'call'))
                                            <th class="text-center"> {{ translate('action') }}</th>
                                        @endif
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach ($DonateLeads as $key => $lead)
                                        <tr>
                                            <td>{{ $DonateLeads->firstItem() + $key }}</td>
                                            <td> <span title="{{ $lead['Trusts']['trust_name'] ?? '' }}" role="tooltip"
                                                    data-toggle="tooltip">
                                                    {{ Str::Limit($lead['Trusts']['trust_name'] ?? '', 20) }} </span>
                                            </td>
                                            <td> <span title="{{ $lead['AdsDonate']['name'] ?? '' }}" role="tooltip"
                                                    data-toggle="tooltip">{{ Str::Limit($lead['AdsDonate']['name'] ?? '', 20) }}
                                                </span> </td>
                                            <td>
                                                <span>{{ $lead['users']['name'] ?? '' }} </span><br>
                                                <span>{{ $lead['users']['phone'] ?? '' }}</span>
                                            </td>
                                            ...
                                            @if (Helpers::modules_permission_check('Donate', 'Lead', 'delete') ||
                                                ...
                                                <td class="text-center">
                                                    ...
                                                    <div class="d-flex justify-content-center gap-2">
                                                        ...
                                                        @if (Helpers::modules_permission_check('Donate', 'Lead', 'delete'))
                                                            <a href="{{ route('admin.donate_management.donate_lead.lead-delete', ['id' => $lead['id']]) }}"
                                                        ...
                                                        @endif
                                                        @if (Helpers::modules_permission_check('Donate', 'Lead', 'follow-up'))
    ...
    <div class="modal fade" id="followUpModal" tabindex="-1" role="dialog">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('admin.donate_management.donate_lead.lead-follow-up') }}" method="POST">
                    @csrf
                    @php
                        if (auth('admin')->check()) {
...
@push('script')
    ...
    <script>
        function followHistory(that) {
            ...
            var row = "";
            $.ajax({
                url: "{{ url('admin/donate_management/donate_lead/donate-follow-list') }}/" + leadId,
                method: 'GET',
                //   data:{id:leadId},
