        ...
        <div class="mb-3">
            <h2 class="h1 mb-0 text-capitalize d-flex align-items-center gap-2">
                <img src="{{ dynamicAsset(path: 'public/assets/back-end/img/deliveryman.png') }}" width="20"
                    alt="">
                {{ translate('delivery_man') }} <span
                    class="badge badge-soft-dark radius-50 fz-12">{{ $deliveryMens->total() }}</span>
            </h2>
        </div>
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-right">
                                        <li>
                                            <a type="submit" class="dropdown-item d-flex align-items-center gap-2 "
                                                href="{{ route('admin.delivery-man.export', ['searchValue' => request('searchValue')]) }}">
                                                <img width="14"
                        ...
                        <table
                            ...
                            <thead class="thead-light thead-50 text-capitalize table-nowrap">
                                <tr>
                                    ...
                                    @if (Helpers::modules_permission_check('Delivery Men', 'Delivery Men', 'status'))
                                        <th class="text-center">{{ translate('status') }}</th>
                                    @endif
                                    @if (Helpers::modules_permission_check('Delivery Men', 'Delivery Men', 'edit') ||
                                            Helpers::modules_permission_check('Delivery Men', 'Delivery Men', 'delete') ||
                                            Helpers::modules_permission_check('Delivery Men', 'Delivery Men', 'statement'))
                                        ...
                                        <td>
                                            <div class="media align-items-center gap-10">
                                                ...
                                                <div class="media-body">
                                                    <a title="Earning Statement" class="title-color hover-c1"
                                        ...
                                        <td>
                                            <div class="d-flex flex-column gap-1">
                                                <div><a class="title-color hover-c1"
                                                        href="mailto:{{ $deliveryMen['email'] }}"><strong>{{ $deliveryMen['email'] }}</strong></a>
                                                </div>
                                                <a class="title-color hover-c1"
                                                    href="tel:{{ $deliveryMen['country_code'] }}{{ $deliveryMen['phone'] }}">{{ $deliveryMen['country_code'] . ' ' . $deliveryMen['phone'] }}</a>
                                            </div>
                                        </td>
                                        <td>
                                            <a href="{{ route('admin.orders.list', ['all', 'delivery_man_id' => $deliveryMen['id']]) }}"
                                                class="badge fz-14 badge-soft--primary">
                                                <span>{{ $deliveryMen->orders_count }}</span>
                                            </a>
                                        </td>
                                        <td>
                                            <a href="{{ route('admin.delivery-man.rating', ['id' => $deliveryMen['id']]) }}"
                                                class="badge fz-14 badge-soft-info">
                                                <span>{{ isset($deliveryMen->rating[0]->average) ? number_format($deliveryMen->rating[0]->average, 2, '.', ' ') : 0 }}
                                                ...
                                                <form action="{{ route('admin.delivery-man.status-update') }}"
                                                    ...
                                                    <label class="switcher mx-auto">
                                                        <input type="checkbox" class="switcher_input toggle-switch-message"
                                                            id="deliveryman_status{{ $deliveryMen['id'] }}" name="status"
                                                            value="1"
                                        ...
                                        @if (Helpers::modules_permission_check('Delivery Men', 'Delivery Men', 'edit') ||
                                            ...
                                            <td>
                                                <div class="d-flex justify-content-center align-items-center gap-10">
                                                    ...
                                                    @endif
                                                    @if (Helpers::modules_permission_check('Delivery Men', 'Delivery Men', 'statement'))
                                                    ...
                                                    @endif
                                                    @if (Helpers::modules_permission_check('Delivery Men', 'Delivery Men', 'delete'))
                                                    ...
                                                    @endif
                                                    <form
                                                        action="{{ route('admin.delivery-man.delete', [$deliveryMen['id']]) }}"
