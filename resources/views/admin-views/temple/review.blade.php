...
@section('title', translate('Temples_review'))
@push('css_or_js')
                    ...
                    <div class="table-responsive">
                        <table id="datatable" class="table table-hover table-borderless table-thead-bordered table-nowrap table-align-middle card-table w-100">
                            <thead class="thead-light thead-50 text-capitalize">
                                <tr>
                                    <th>{{ translate('SL') }}</th>
                                    <th class="text-center">{{ translate('Temple_name') }} </th>
                                    ...
                                    @if (Helpers::modules_permission_check('Temple', 'Temple', 'review-status') || Helpers::modules_permission_check('Temple', 'Temple', 'review-delete'))
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center"><span title="{{ ($videosubcategory['templeData'][0]['name'] ?? '') }}" role="tooltip" data-toggle="tooltip">{{ Str::limit(($videosubcategory['templeData'][0]['name'] ?? ''),20) }}</span></td>
                                    <td class="text-center">{{ ($videosubcategory['userData'][0]['name'] ?? '') }}</td>
                                    <td class="text-center">  
                                            @if($videosubcategory['image'])
                                        <img  alt="" class="h-auto aspect-1 bg-white onerror-add-class-d-none" src="{{ getValidImage(path: 'storage/app/public/temple/review/' . $videosubcategory['image'], type: 'backend-product') }}" style="width: 100px;" />
                                    @endif
                                    </td>
                                    ...
                                    <td>
                                        <div class="d-flex justify-content-center gap-2">  
                                            @if (Helpers::modules_permission_check('Temple', 'Temple', 'review-status'))
                                        <form action="{{route('admin.temple.review-status-update') }}" method="post" id="review-status_change{{$videosubcategory['id']}}-form">
                                            @csrf
                                        ...
                                        @if (Helpers::modules_permission_check('Temple', 'Temple', 'review-delete'))
                                            <a class="delete-data btn btn-outline-danger btn-sm square-btn" data-id="cities_visit-{{ $videosubcategory['id'] }}">
                                                <i class="tio-delete"></i>
                                            </a>
                                            <form action="{{ route('admin.temple.review-delete',$videosubcategory['id'] )}}" method="post" id="cities_visit-{{ $videosubcategory['id'] }}">
                                                @csrf 
