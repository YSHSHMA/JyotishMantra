...
@section('title', translate('restaurant_gallery'))
@section('content')
            ...
            <div class="card">
                <div class="card-body text-start">
                    <form id="form_submit_data" method="post" enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" name="restaurant_id" value="{{ $gallery_list['id'] }}">
                        <div class="row">
                            <div class="col-md-12">
                                <div class="row">
                                    <div class="additional_image_column col-md-12">
                                        ...
                                        <div class="coba-area">
                                            ...
                                            <div class="row g-2" id="additional_Image_Section">
                                                ...
                                                @if (isset($gallery_list['images']) && !empty($gallery_list['images']) && json_decode($gallery_list['images']))
                                                @foreach (json_decode($gallery_list['images']) as $unique_id => $photo)
                                                <div class="col-sm-12 col-md-4">
                                                    <div class="custom_upload_input custom-upload-input-file-area position-relative border-dashed-2">

                                                        <a class="delete_file_input_css btn btn-outline-danger btn-sm square-btn" href="{{ route('admin.temple.restaurants.delete-image', ['id' => $gallery_list['id'], 'name' => $photo]) }}">
                                                            <i class="tio-delete"></i>
                                                        </a>
                                                        <div class="img_area_with_preview position-absolute z-index-2 border-0">
                                                            <img id="additional_Image_{{ $unique_id }}" alt="" class="h-auto aspect-1 bg-white onerror-add-class-d-none" src="{{ getValidImage(path: 'storage/app/public/temple/restaurant/' . $photo, type: 'backend-product') }}">
                                                        </div>
                                                        <div class="position-absolute h-100 top-0 w-100 d-flex align-content-center justify-content-center">
                                                            <div class="d-flex flex-column justify-content-center align-items-center">
                                                                <img alt="" src="{{ dynamicAsset(path: 'public/assets/back-end/img/icons/product-upload-icon.svg') }}" class="w-75">
...
<script>
    ...
    function filesManager(that) {
        ...
        $.ajax({
            url: '{{ route("admin.temple.restaurants.gallery_add",[$gallery_list["id"]])}}',
            data: formdata,
            dataType: "json",
            beforeSend: function () {
            $('#loading').fadeIn();
