...
<div class="content container-fluid">
    ...
    <div class="row">
        ...
        <div class="col-md-12 mb-3">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('admin.temple.hotel.edit',[$getData['id']]) }}" method="post" enctype="multipart/form-data">
                        @csrf
                        <!-- Language tabs -->
                        ...
                        <div class="col-12">
                            ...
                            if (count($getData['translations'])) {
                                ...
                                foreach ($getData['translations'] as $translation) {
                                    if ($translation->locale == $lang && $translation->key == 'hotel_name') {
                                        $translate[$lang]['hotel_name'] = $translation->value;
                                    }
                                    if ($translation->locale == $lang && $translation->key == 'description') {
                                        $translate[$lang]['description'] = $translation->value;
                                    }
                                    if ($translation->locale == $lang && $translation->key == 'amenities') {
                                        $translate[$lang]['amenities'] = $translation->value;
                                    }
                                    if ($translation->locale == $lang && $translation->key == 'room_amenities') {
                                        $translate[$lang]['room_amenities'] = $translation->value;
                                    }
                                    if ($translation->locale == $lang && $translation->key == 'room_types') {
                                        $translate[$lang]['room_types'] = $translation->value;
                                    }
                                    if ($translation->locale == $lang && $translation->key == 'booking_information') {
                                        $translate[$lang]['booking_information'] = $translation->value;
                                    }
                                }
                            }
                            ?>
                            <div class="row {{$lang != $defaultLanguage ? 'd-none':''}} form-system-language-form" id="{{$lang}}-form">
                                <div class='col-md-6 form-group'>
                                    <label class="title-color" for="hotel_name">{{ translate('Hotel_name') }}<span class="text-danger">*</span>
                                        ({{ strtoupper($lang) }})</label>
                                    <input type="text" name="hotel_name[]" value="{{ $translate[$lang]['hotel_name'] ?? $getData['hotel_name'] }}" class="form-control getAddress_google" id="{{$lang}}_hotel_name" autocomplete="off" placeholder="{{ translate('Hotel_name') }}" required="{{ $lang == $defaultLanguage? 'required':''}}" data-option='{{$key}}'>
                                </div>
                                ...
                                <div class="col-md-6 form-group">
                                    ...
                                    <select name="country_id" class="js-select2-custom form-control Country_select_geolocation">
                                        ...
                                        <option value="{{$va->id}}" {{ (($va->id == $getData['country_id'])?"selected":"") }}>{{ $va->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="col-md-6 form-group">
                                    ...
                                    <select name="cities_id" class="js-select2-custom form-control City_select_geolocation">
                                        ...
                                        <option value="{{$va->id}}" {{ (($va->id == $getData['cities_id'])?"selected":"") }}>{{ $va->city}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="col-md-6 form-group">
                                    ...
                                    <select name="state_id" class="js-select2-custom form-control state_select_geolocation">
                                        ...
                                        <option value="{{$va->id}}" {{ (($va->id == $getData['state_id'])?"selected":"") }}>{{ $va->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-md-6 form-group">
                                    <label class="title-color" for="name">{{ translate('zipcode') }}<span class="text-danger">*</span>
                                        ({{ strtoupper($lang) }})</label>
                                ...
                                <div class="col-md-6 form-group">
                                    <label class="title-color" for="phone_number">{{ translate('phone_number') }}<span class="text-danger">*</span>
                                        ({{ strtoupper($lang) }})</label>
                                ...
                                <div class="col-md-6 form-group">
                                    <label class="title-color" for="email">{{ translate('email_id') }}<span class="text-danger">*</span>
                                        ({{ strtoupper($lang) }})</label>
                                    <input type="email" name="email_id" class="form-control showallData" value="{{ $getData['email_id'] }}" data-point='2' id="{{$lang}}_email" placeholder="{{ translate('email_id') }}" autocomplete="off" required="{{$lang == $defaultLanguage? 'required':''}}">
                                    <input type="hidden" name="lang[]" value="{{$lang}}" id="lang">
                                </div>
                                <div class="col-md-6 form-group">
                                    <label class="title-color" for="email">{{ translate('Website_link') }}<span class="text-danger">*</span>
                                        ({{ strtoupper($lang) }})</label>
                                ...
                                <div class="col-md-6 form-group">
                                    <label class="title-color" for="name">{{ translate('Amenities') }}<span class="text-danger">*</span> ({{ strtoupper($lang) }})</label>
                                    <textarea class="ckeditor form-control" name="amenities[]">{{ $translate[$lang]['amenities'] ?? $getData['amenities'] }}</textarea>
                                </div>
                                <div class="col-md-6 form-group">
                                    <label class="title-color" for="name">{{ translate('Room_Amenities') }}<span class="text-danger">*</span> ({{ strtoupper($lang) }})</label>
                                    <textarea class="ckeditor form-control" name="room_amenities[]">{{ $translate[$lang]['room_amenities'] ?? $getData['room_amenities'] }}</textarea>
                                </div>
                                <div class="col-md-6 form-group">
                                    <label class="title-color" for="name">{{ translate('room_types') }}<span class="text-danger">*</span> ({{ strtoupper($lang) }})</label>
                                    <textarea class="ckeditor form-control" name="room_types[]" id="room_types">{{ $translate[$lang]['room_types'] ?? $getData['room_types'] }}</textarea>
                                </div>
                                <div class="col-md-12 form-group">
                                    <label class="title-color" for="name">{{ translate('booking_information') }}<span class="text-danger">*</span> ({{ strtoupper($lang) }})</label>
                                    <textarea class="ckeditor form-control" name="booking_information[]" id="booking_information">{{ $translate[$lang]['booking_information'] ?? $getData['booking_information'] }}</textarea>
                                </div>
                        ...
                        <div class="row">
                                    ...
                                    <div class="card-body">
                                        <div class="form-group">
                                            <div class="d-flex align-items-center justify-content-between gap-2 mb-3">
                                                <div>
                                                    <label for="name"
                                                        class="title-color text-capitalize font-weight-bold mb-0">{{ translate('hotel_thumbnail') }}</label>
                                                    <span
                                                        class="badge badge-soft-info">{{ THEME_RATIO[theme_root_path()]['Product Image'] }}</span>
                                                    <span class="input-label-secondary cursor-pointer" data-toggle="tooltip"
                                                        title="{{ translate('add_your_hotel_service’s_thumbnail_in') }} JPG, PNG or JPEG {{ translate('format_within') }} 2MB">
                                                ...
                                                <div class="custom_upload_input">
                                                    ...
                                                    <div class="img_area_with_preview position-absolute z-index-2">
                                                        <img id="pre_img_viewer" class="h-auto aspect-1 bg-white  {{ (($getData['image'])?'':'d-none') }}"
                                                            src="{{ getValidImage(path: 'storage/app/public/temple/hotel/' .$getData['image'] , type: 'backend-product') }}" alt="">
                                                    </div>
                                                    <div
                            ...
                            <div class="col-md-9">
                                <div class="d-flex gap-2">
                                    ...
                                    <h4 class="mb-0">{{ translate('hotel_video') }}</h4>
                                    <span class="input-label-secondary cursor-pointer" data-toggle="tooltip"
                                ...
                                <input type="text" name="youtube_video" value="{{$getData['youtube_video']??''}}"
