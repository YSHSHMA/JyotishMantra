...
@section('title', translate('VIP_darshan_lead'))
@push('css_or_js')
                ...
                <div class="card-body p-0">
                    <div class="table-responsive">
                        <table
                            ...
                            <tbody>
                                @foreach ($darshanlist as $key => $lead)
                                <tr>
                                    <td>{{ $key + 1 }}</td>
                                    <td>{{ $lead['lead_id'] }}</td>
                                    <td>
                                        <span class="h4 font-weight-bolder">{{ $lead['title'] }}</span><br>
                                        <span class="h6 font-weight-bolder">{{ $lead['package_name'] }}</span>
                                    </td>
                                    <td>{{ $lead['name'] }}<br>
                                        {{ $lead['phone'] }}<br>
                                        Amount:
                                        {{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $lead['price']??0), currencyCode: getCurrencyCode()) }}
                                    </td>
                                    ...
                                    <td>
                                        <div class="d-flex justify-content-center gap-2">
                                            <a href="{{ route('admin.temple.darshan-leads.leads-delete', ['id' => $lead['id']]) }}" class="btn btn-icon bg-label-danger waves-effect waves-light myactionbtn" onclick="return confirm('Are your sure, you want to delete');" data-toggle="tooltip" aria-label="Delete" data-bs-original-title="Delete" title="Delete">
                                                <i class="tio-delete-outlined"></i>
                                            </a>
                                            <a href="javascript:void(0)" class="btn btn-icon bg-label-success waves-effect waves-light myactionbtn" data-leadsId="{{ $lead['id'] }}"
                                                onclick="followUp(this)" data-toggle="tooltip" aria-label="Follow Up" data-bs-original-title="Follow Up" title="Follow Up">
                                                <i class="tio-settings-back"></i>
                                            </a>
                                        </div>
                                        <div class="d-flex justify-content-center gap-2 pt-2">
                                            <a href="javascript:0" class="btn btn-icon bg-label-info waves-effect waves-light myactionbtn" data-leadsId="{{ $lead['id'] }}" onclick="followHistory(this)" data-toggle="tooltip" aria-label="Follow Up History" data-bs-original-title="Follow Up History" title="Follow Up History">
                                                <i class="tio-history"></i>
                                            </a>
                                            <a href="tel:{{ $lead['phone'] }}" class="btn btn-icon bg-label-warning waves-effect waves-light myactionbtn" data-toggle="tooltip" aria-label="Call" data-bs-original-title="Call">
                                                <i class="tio-call" title="Call"></i>
                                            </a>
                                        </div>
                                        <div class="d-flex justify-content-center gap-2 pt-2">
                                            <a href="{{ route('admin.temple.darshan-leads.send-whatsapp-leads', ['id' => $lead['id']]) }}" class="btn btn-icon bg-label-success waves-effect waves-light myactionbtn" data-toggle="tooltip" aria-label="whatsapp" data-bs-original-title="whatsapp"><i class="tio-whatsapp" title="whatsapp"></i>
                                                <span class="btn-status btn-sm-status btn-status-danger">{{ $lead['whatsapp_hit'] }}</span>
                                            </a>
                                            <a href="{{ route('admin.whatsapp.temple-darshan-template') }}"
...
<div class="modal fade" id="followUpModal" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <form action="{{ route('admin.temple.darshan-leads.darshan-follow-up') }}" method="POST">
                @csrf
                @php
                if (auth('admin')->check()) {
...
<script>
    function followHistory(that) {
        ...
        var row = "";
        $.ajax({
            url: "{{ route('admin.temple.darshan-leads.darshan-follow-list') }}/" + leadId,
            method: 'GET',
