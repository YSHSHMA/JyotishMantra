        ...
        @foreach ($combinations as $key => $combination)
            ...
            @if(strlen($str) > 0)
                <tr>
                    ...
                    <td>
                        <input type="number" name="price_{{ $str }}" value="{{ ((!empty($request['price_'.$str]))?$request['price_'.$str]:$unitPrice) }}" min="0" step="0.01"
                               class="form-control variation-price-input remove-symbol" required
                               placeholder="{{ translate('ex').': 100' }}">
                    </td>
                    <td>
                        <input type="text" name="sku_{{ $str }}" value="{{ ((!empty($request['sku_'.$str]))?$request['sku_'.$str]:strtoupper($sku)) }}" class="form-control" required
                               placeholder="{{ translate('ex').': MCU-47-V593-M' }}">
                    </td>
                    <td>
                        <input type="number" name="qty_{{ $str }}" value="{{ ((!empty($request['qty_'.$str]))?$request['qty_'.$str]:1) }}" min="1"
