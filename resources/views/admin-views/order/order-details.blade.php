...
@section('content')
                        ...
                        <div class="d-flex flex-wrap flex-md-nowrap gap-10 justify-content-between mb-4">
                            ...
                            <div class="text-sm-right flex-grow-1">
                                <div class="d-flex flex-wrap gap-10 justify-content-end">
                                    ...
                                    @endif
                                    @if (Helpers::modules_permission_check('Product Order', 'Detail', 'show-location'))
                                    ...
                                    @if (Helpers::modules_permission_check('Product Order', 'Detail', 'print-invoice'))
            ...
            <div class="col-lg-4 d-flex flex-column gap-3">
                ...
                <div class="card">
                    <div class="card-body text-capitalize d-flex flex-column gap-4">
                        ...
                        @if (Helpers::modules_permission_check('Product Order', 'Detail', 'order-status'))
                            <div class="">
                                <label
                                ...
                                <select name="order_status" id="order_status" class="status form-control"
                                    ...
                                    @if ($order->order_status == 'out_for_delivery' || $order->order_status == 'pickup')
                        ...
                        @if (Helpers::modules_permission_check('Product Order', 'Detail', 'payment-status'))
                            <div
                        ...
                        @if (Helpers::modules_permission_check('Product Order', 'Detail', 'shipping-method'))
                            {{-- @if (!$physicalProduct) --}}
                            @if ($order['delivery_type'] == 'self_delivery' || $order['delivery_partner'] == 'self_delivery')
                            @endif
                            {{-- @endif --}}
                        @endif
                    </div>
                        ...
                        <div class="card-body">
                                    ...
                                    @if (isset($order->deliveryMan))
                                        <li class="choose_delivery_man">
                                            <label class="font-weight-bold title-color d-flex fz-14">
                                                {{ translate('delivery_man_incentive') }}
                            ...
                            @else
                                <div class="d-flex flex-column gap-2">
                                    <?php
                                    $pickupdata = App\Models\Order_Pickup::where('order_ids', $order['delivery_order_id'])->first();
                                    ?>
                                    @if ($pickupdata)
                                        <div class="d-flex">
                                            <div>
                                                <span>{{ translate('Weight') }}: </span>
                                                <strong>{{ $order['order_weight'] }}</strong>
                                            </div>

                                            <div>
                                                <span>{{ translate('Length') }}: </span>
                                                <strong>{{ $order['box_length'] }}</strong>
                                            </div>
                                            <div>
                                                <span>{{ translate('Breath') }}: </span>
                                                <strong>{{ $order['box_breadth'] }}</strong>
                                            </div>
                                            <div>
                                                <span>{{ translate('Height') }}: </span>
                                                <strong>{{ $order['box_height'] }}</strong>
                                            </div>
                                        </div>
                                        <div>
                                            <span>{{ translate('Carrier_Name') }}: </span>
                                            <strong> {{ $pickupdata['courier_name'] ?? 'N/A' }}
                                                ({{ $pickupdata['carrier_id'] ?? 'N/A' }})
                                            </strong><br>
                                            <span>{{ translate('delivery_charge') }}: </span>
                                            <strong> Rs.{{ $pickupdata['delivery_charge'] }}/- </strong><br>
                                            <span class="fz-12 title-color">{{ translate('AWB') }} : <b>
                                                    {{ $order->third_party_delivery_tracking_id ?? translate('Watting_for_tracking_number') }}
                                                </b></span><br>
                                            <span class="fz-12 title-color">{{ translate('Manifest_ID') }} : <b>
                                                    {{ $order->manifest_id ?? translate('Watting_for_manifest_id') }}
                                                </b></span><br>
                                        ...
                                        <div class="d-flex">
                                            @if ($order->third_party_delivery_tracking_id)
                                                <a href="{{ $order?->shippingurl ?? translate('mahakal.com') }}"
                                                    target="_blank" class="btn btn-info btn-sm mr-2"
                                                    title="invoice Downlaod"><i class="tio-download-to"></i></a>
                                                <form id="delivery-cancel-form"
                                                    action="{{ route('admin.orders.delivery.shipmentcancel') }}"
                                                    method="POST">
                                                    @csrf
                                    ...
                                    <div class="row gx-2">
                                        ...
                                        <div class="col-12 ">
                                            <input id="billing-pac-input" class="form-control rounded __map-input mt-1"
                                                title="{{ translate('search_your_location_here') }}" type="text"
                                                placeholder="{{ translate('search_here') }}" />
                                            <div class="rounded w-100 __h-200px mb-5" id="location_map_canvas_billing">
                                            </div>
                                        </div>
                                        <div class="col-12">
                                            <div class="d-flex justify-content-end gap-3">
    ...
    <div class="modal fade" id="packaging" tabindex="-1" aria-labelledby="packagingLabel" aria-hidden="true"
        ...
        <div class="modal-dialog modal-">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="packagingLabel">Order Assign Details</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form id="delivery-partner-form" action="#" method="post">
                        ...
                        <input type="hidden" id="frompincode" name="fromPincode"
                            value="{{ optional(optional($order->seller)->shop)->pincode }}" class="form-control">
                        ...
                        <input type="hidden" name="warehouse_id" id="warehouseid"
                            value="{{ optional($order->seller)->warehouse_id }}" class="form-control">
                        ...
                        <div class="row justify-content-end gap-3 mt-3 mx-1">
                            <button type="button" class="btn btn--primary px-4" id="orderUpdate"
                                onclick="AdminOrderDetails()">{{ translate('order_pickup') }}</button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    {{-- <div class="modal fade" id="pickupModal" tabindex="-1" aria-labelledby="pickupModalLabel" aria-hidden="true"
        ...
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="pickupModalLabel">Order Pickup Details</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form id="delivery-pickup-form" action="#" method="post">
                        @csrf
                        <input type="hidden" id="id" name="id" value="{{ $order['id'] }}" class="form-control">
                        <input type="hidden" name="order_status" value="pickup" class="form-control">
                        <input type="hidden" name="warehouse_id" id="warehouseid" value="{{ $order->seller->warehouse_id }}" class="form-control">
                        <input type="hidden" name="return_warehouse_id" id="warehouseidr" value="{{ $order->seller->warehouse_id }}" class="form-control">
                        <div class="row">
                            <div class="form-group col-md-6">
                        ...
                        <div class="row">
                            <div class="form-group col-md-6">
                                ...
                                <input type="time" id="officetime" name="office_close_time" class="form-control w-100"
                                    placeholder="office close time" required>
                            </div>
                            <div class="form-group col-md-6">
                                <label class="title-color">{{ translate('package_count') }}</label>
                                <input type="number" id="packagecouunt" name="package_count" class="form-control w-100" placeholder="package count" min="1" required>
                            </div>
                        </div>
                        <div class="row justify-content-end gap-3 mt-3 mx-1">
                            <button type="button" class="btn btn--primary px-4" id="pickupOrder" onclick="AdminOrderPickup()">{{ translate('order_pickup') }}</button>
                        </div>
                    </form>
                </div>
            </div>
...
@push('script_2')
    ...
    {{-- deliver partner --}}
    {{-- <script>
        $('#delivery-partner').change(function(e) {
            e.preventDefault();
    ...
    <script>
        ...
        $('#delivery-cancel').on('click', function() {
            Swal.fire({
                title: 'Are You Sure To Cancel Delivery Order',
                type: 'success',
    ...
    <script>
        $('#delivery-pickup').on('click', function() {
            const fromPincode = $(this).data('frompincode');
            const toPincode = $(this).data('topincode');
            const paymentType = $(this).data('paymenttype');
            $.ajax({
                url: '{{ url('admin/orders/delivery/getcarries') }}/' + fromPincode + '/' + toPincode +
                    '/' + paymentType,
                type: 'GET',
                success: function(response) {
                    console.log(response);
                    if (response.rate_card && Array.isArray(response.rate_card) && response.rate_card
                        .length > 0) {
                        let dropdownOptions =
                            ' <label class="title-color">Delivery Partner</label><select class="form-control" id="carrierDropdown" name="carrier_id">';
                        response.rate_card.forEach(function(carrier) {
                            dropdownOptions += `
                                <option value="${carrier.carrier_id}">
                                    ${carrier.courier_name} - Delivery Charge: ${carrier.delivery_charge} (COD: ${carrier.cod_charges})
                                </option>
                            `;
                        });
                        dropdownOptions += '</select>';
                        $('#modal-api-response').html(`${dropdownOptions}`);
                        $('#pickupModal').modal('show');
                    } else {
                        $('#modal-api-response').html(
                            '<p class="text-danger">No carrier data available</p>');
                        $('#pickupModal').modal('show');
                    }
                },
