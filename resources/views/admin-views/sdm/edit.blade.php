...
@section('title', translate('SDM_Edit'))
        ...
        <div class="row">
            ...
            <div class="card mb-3">
                <div class="card-body text-start">
                    <form action="{{ route('admin.sdm.update') }}" method="post" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select id="collector" name="collector_id" class="form-control" disabled required>
                                        @forelse ($collectors as $collector)
                                            <option value="{{$collector->id}}" {{$collector->id==$edit->collector_id?'selected':''}}>{{$collector->name}}</option>                                            
                                        @empty
    ...
    <script>
        $(document).ready(function() {
            ...
            function loadTemples() {
                ...
                $.ajax({
                    url: "{{ route('admin.sdm.get-selected-temple') }}",
                    type: "GET",
                    data: {
                        sdmId: sdmId,
                        collector: collector
