                        ...
                        <div class="col-sm-6 col-md-3">
                            <div class="">
                                <select class="js-select2-custom form-control __form-control" name="seller_id">
                                    ...
                                    <option class="text-center" value="inhouse"
                                        {{ $seller_id == 'inhouse' ? 'selected' : '' }}>
                                        {{ translate('inhouse') }}
                                    </option>
                                    @foreach ($sellers as $seller)
                                        <option class="text-left text-capitalize" value="{{ $seller->id }}"
                                            {{ $seller->id == $seller_id ? 'selected' : '' }}>
                    ...
                    <form action="{{ url()->full() }}" method="GET" class="mb-0">
                        <div class="input-group input-group-merge input-group-custom">
                            ...
                            <input id="datatableSearch_" type="search" name="search" class="form-control"
                                placeholder="{{ translate('search_by_orders_id') }}" aria-label="Search orders"
                    ...
                    @endphp
                            @if ($transaction->order)
                                <tr>
                                    ...
                                    <td>
                                        {{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $transaction->orderDetails[0]->order_details_sum_price), currencyCode: getCurrencyCode()) }}
                                    </td>

                                    <td>
                                        {{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $transaction->orderDetails[0]->order_details_sum_discount), currencyCode: getCurrencyCode()) }}
                                    </td>
                                    ...
                                    <td>
                                        {{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $transaction->orderDetails[0]->order_details_sum_price - $transaction->orderDetails[0]->order_details_sum_discount - (isset($transaction->order->coupon) && $transaction->order->coupon->coupon_type != 'free_delivery' ? $transaction->order->discount_amount : 0)), currencyCode: getCurrencyCode()) }}
                                    </td>
                                    ...
                                    <td>
                                        {{ \App\Models\OrderDetail::where('order_id', $transaction['order_id'])->value('tax_model') }}
                                    </td>
                                    ...
                                    if (!empty($transaction->order->delivery_charge)) {
                                        $remainingAmt = $transaction->order->shipping_cost - $transaction->order->delivery_charge;
                                    }
                                @endphp
