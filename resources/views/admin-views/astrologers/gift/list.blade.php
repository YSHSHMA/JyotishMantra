                    ...
                    <div class="px-3 py-4">
                        <div class="row g-2 flex-grow-1">
                            ...
                            <div class="col-sm-4 col-md-6 col-lg-8 d-flex justify-content-end">
                                <a href="{{route('admin.astrologers.gift.add-new')}}" type="button" class="btn btn-outline--primary">
                                    {{ translate('add_Astrologer_Gift') }}
                                </a>
                            </div>
                            @endif
                        </div>
                        ...
                        <div class="table-responsive">
                            <table
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td>{{$key+1}}</td>
                                        <td> <img src="{{ asset('storage/app/public/astrologer-gift-img/' . $gift['image']) }}"
                                            alt="" width="50"></td>
                                        <td>{{$gift['name']}}</td>
                                        <td>{{$gift['amount']}}</td>
                                        @if (Helpers::modules_permission_check('Astrologer & Pandit', 'Gift', 'status'))
                                        <td>
                                            <div class="custom-control custom-switch">
                                                <input type="checkbox" class="custom-control-input gift-checkbox" {{$gift['status'] == 1 ? "checked" : ""}} id="customSwitch{{$gift['id']}}" data-id="{{$gift['id']}}">
                                                <label class="custom-control-label" for="customSwitch{{$gift['id']}}"></label>
                                            </div>
                                        </td>
                                        @endif
                                        @if (Helpers::modules_permission_check('Astrologer & Pandit', 'Gift', 'edit'))
                                        <td>
                                            <div class="d-flex justify-content-start gap-2">
                                                <a class="btn btn-outline-info btn-sm square-btn"
                                                    title="{{ translate('edit') }}" href="{{ route('admin.astrologers.gift.update', [$gift['id']]) }}">
                                                    <i class="tio-edit"></i>
                                                </a>
                                            </div>
                                        </td>
...
@push('script')
    ...
    <script>
        $('.gift-checkbox').change(function() {
           ...
           $.ajax({
               url: "{{ route('admin.astrologers.gift.status') }}",
               type: "POST",
               data: {
                   _token: '{{ csrf_token() }}',
                   id: id,
