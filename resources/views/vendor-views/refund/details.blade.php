...
@section('content')
    ...
    <div class="content container-fluid">
        <div class="mb-3">
            <h2 class="h1 mb-0 text-capitalize d-flex align-items-center gap-2">
                <img width="20" src="{{ dynamicAsset(path: 'public/assets/back-end/img/refund-request-list.png') }}"
                    alt="">
                    ...
                    <div class="d-flex justify-content-between w-100">
                        <span class="">
                            ...
                            @if ($refund['status'] != 'pending' && ($refund['approved_count'] < 2 || $refund['denied_count'] < 2))
                                ...
                                @elseif($refund['status'] == 'received' && $refund['denied_count'] < 2)
                                    {{ translate('you_have_product_recevide_for_vendor') . '.' }}
                                @endif
                            ...
                            <ul class="dm-info p-0 m-0">
                                <li class="align-items-center">
                                    <span class="left">{{ translate('refund_id') }} </span> <span>:</span> <span
                                        class="right">{{ $refund->id }}</span>
                                </li>
                                <li class="align-items-center">
                                    <span class="left text-capitalize">{{ translate('refund_requested_date') }}</span>
                                    <span>:</span>
                                    <span
                                        class="right">{{ date('d M Y, h:s:A', strtotime($refund['created_at'])) }}</span>
                                </li>
                                <li class="align-items-center">
                                    <span class="left">{{ translate('refund_status') }}</span> <span>:</span> <span
                                        class="right">
                                        @if ($refund['status'] == 'pending')
                                            <span class="badge badge-secondary-2">
                                                {{ translate($refund['status']) }}</span>
                                        ...
                                        @elseif($refund['status'] == 'received')
                                            <span class="badge badge--info-2"> {{ translate($refund['status']) }}</span>
                                        @endif
                                    </span>
                                </li>
                                <li class="align-items-center">
                                ...
                                @if ($order->delivery_partner == 'shipway')
                                    <li class="align-items-center">
                                        <span class="left">{{ translate('rma_no') }} </span> <span>:</span> <span
                                            class="right">
                                            {{ $refund['rma_no'] }}</span>
                                    </li>
                                    <li class="align-items-center">
                                        <span class="left">{{ translate('Return AWB') }} </span> <span>:</span> <span
                                            class="right">
                                            {{ $order->third_party_delivery_tracking_id }}</span>
                                    </li>
                                @endif
                                <li class="align-items-center">
                                    <span class="left">{{ translate('order_details') }}({{ $refund['order_id'] }})
                                    </span> <span>:</span> <span class="right"><a
                                            class="badge py-2 badge-soft-primary border border-primary px-2"
                                            href="{{ route('vendor.orders.details', ['id' => $order->id]) }}">{{ translate('view_details') }}</a></span>
                                </li>
                            </ul>
                        </div>
                    </div>
                                ...
                                <div class="d-flex flex-wrap gap-3">
                                    @if ($refund->change_by != 'admin')
                                        @if ($refund['status'] != 'rejected' && $refund['denied_count'] < 2)
                                            <button class="btn btn-soft-danger p-2 px-3" data-toggle="modal"
                                                data-target="#rejectModal">
                                                {{ translate('reject') }}
                                            </button>
                                        @endif
                                        @if ($refund['status'] != 'approved' && $refund['approved_count'] < 2)
                                            <button class="btn btn-soft-success p-2 px-3" data-toggle="modal"
                                                data-target="#approveModal">
                                                {{ translate('approve') }}
                                            </button>
                                        @endif
                                        @if ($refund['status'] != 'received')
                                            <button class="btn btn-soft-primary p-2 px-3" data-toggle="modal"
                                                data-target="#receivedModel">{{ translate('Product received') }}
                                            </button>
                                        @endif
                                    @endif
                                </div>
                            ...
                            <div class="refund-details">
                                <div class="img">
                                    <div class="onerror-image border rounded">
                                        <img src="{{ getValidImage(path: 'storage/app/public/product/thumbnail/' . ($refund->product ? $refund->product->thumbnail : ''), type: 'backend-product') }}"
                                            alt="">
                                    </div>
                                </div>
                                <div class="--content flex-grow-1">
                                    ...
                                    @if ($refund->orderDetails->digital_file_after_sell)
                                        <a href="{{ dynamicStorage(path: 'storage/app/public/product/digital-product/' . $refund->orderDetails->digital_file_after_sell) }}"
                                ...
                                <ul class="dm-info p-0 m-0 w-l-115">
                                    ...
                                    <li>
                                        ...
                                        <span class="right">
                                            <strong>
                                                {{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: ($refund->orderDetails->price ?? 0) * ($refund->orderDetails->qty ?? 0)), currencyCode: getCurrencyCode()) }}
                                            </strong>
                                        </span>
                                    </li>
                                    ...
                                    <li>
                                        ...
                                        <span class="right">
                                            <strong>
                                                {{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $refund->orderDetails->discount ?? 0), currencyCode: getCurrencyCode()) }}
                                            </strong>
                                        </span>
                                    </li>
                                    <li>
                                        ...
                                        <span class="right">
                                            <strong>
                                                {{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $couponDiscount ?? 0), currencyCode: getCurrencyCode()) }}
                                            </strong>
                                        </span>
                                    </li>
                                    ...
                                    <li>
                                        ...
                                        <span class="right">
                                            <strong>
                                                {{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $refund->orderDetails->tax ?? 0), currencyCode: getCurrencyCode()) }}
                                            </strong>
                                        </span>
                                    </li>
                                    ...
                                    <li>
                                        ...
                                        <span class="right">
                                            <strong>
                                                {{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $subtotal ?? 0), currencyCode: getCurrencyCode()) }}
                                            </strong>
                                        </span>
                                    </li>
                                    <li>
                                        <span class="left">{{ translate('shipping_charge') }} </span>
                                        <span>:</span>
                                        <span class="right">
                                            <strong>
                                                {{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $shippingcharge ?? 0), currencyCode: getCurrencyCode()) }}
                                            </strong>
                                        </span>
                                    </li>
                                    <li>
                                        ...
                                        <span class="right">
                                            <strong>
                                                {{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $refundAmount ?? 0), currencyCode: getCurrencyCode()) }}
                                            </strong>
                                        </span>
                                    </li>
                ...
                <div class="col-sm-6">
                    <div class="card h-100 refund-details-card--2">
                        <div class="card-body">
                            ...
                            <p>Customer Name:<b>{{ $order->shipping_address_data->contact_person_name }}</b></p>
                            <p>Customer Address:<b>{{ $order->shipping_address_data->address }}
                                    {{ $order->shipping_address_data->city }} {{ $order->shipping_address_data->zip }}</b>
                            </p>
                            <p>product Reason & Image :
                                {{ $refund->reason_data }} ( {{ $refund->reason_name }} )<br>
                            <p>
                            <p>Customer Message:{{ $refund->refund_reason }}</p>
                            </p>
                            @if ($refund->images)
                                <div class="gallery grid-gallery">
                                    @foreach (json_decode($refund->images) as $key => $photo)
                                        <a href="{{ getValidImage(path: 'storage/app/public/refund/' . $photo, type: 'backend-basic') }}"
                                            ...
                                            <img src="{{ getValidImage(path: 'storage/app/public/refund/' . $photo, type: 'backend-basic') }}"
                                                width="50" alt="">
                                        </a>
                                    @endforeach
                                </div>
                        ...
                        <div class="card-body">
                            ...
                            <div class="key-val-list d-flex flex-column gap-2 min-width--60px">
                                ...
                                @if ($order->deliveryMan)
                                    ...
                                    <div class="key-val-list-item d-flex gap-3">
                                        <span class="text-capitalize">{{ translate('email') }}</span>:
                                        <span>
                                            <a class="text-dark"
                                ...
                                @elseif ($refund->patner_type == 'shipway')
                                    <div class="key-val-list-item d-flex gap-3">
                                        <span class="text-capitalize">{{ translate('Tracking_id') }}</span>:
                                        <span>
                                            <a class="text-dark"
                                                href="javascript:void(0)">{{ $order->third_party_delivery_tracking_id }}
                                            </a>
                                        </span>
                                    </div>
                                    <div class="key-val-list-item d-flex gap-3">
                                        <span class="text-capitalize">{{ translate('carrier_id') }} </span>:
                                        <span>
                                            <a class="text-dark"
                                                href="javascript:void(0)">{{ $order->delivery_shipment_id }}
                                            </a>
                                        </span>
                                    </div>
                                @endif
    ...
    @if ($refund['change_by'] != 'admin')
            ...
            <div class="modal fade" id="rejectModal">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <form action="{{ route('vendor.refund.update-status') }}" method="post"
                            ...
                            <div class="modal-body">
                                ...
                                <div class="text-center">
                                    <img class="mb-3"
                                        src="{{ dynamicAsset(path: 'public/assets/back-end/img/refund-reject.png') }}"
                                ...
                                <textarea class="form-control text-area-max-min"
                                    placeholder="{{ translate('please_write_the_reject_reason') . '...' }}" name="rejected_note" rows="3"></textarea>
                                <div class="d-flex flex-wrap justify-content-end gap-3 mt-3">
                                    ...
                                    <button type="button" class="btn btn--primary form-submit"
                                        ...
                                        data-redirect-route="{{ route('vendor.refund.index', ['status' => $refund['status']]) }}">{{ translate('submit') }}</button>
                                </div>
                            </div>
                        </form>
                    </div>
                    ...
                    <div class="modal-content">
                        <form action="{{ route('vendor.refund.update-status') }}" method="post"
                            ...
                            <div class="modal-body">
                                ...
                                <div class="text-center">
                                    <img class="mb-3"
                                        src="{{ dynamicAsset(path: 'public/assets/back-end/img/refund-approve.png') }}"
                                ...
                                <textarea class="form-control text-area-max-min"
                                    placeholder="{{ translate('please_write_the_approve_reason') . '...' }}" name="approved_note" rows="3"></textarea>
                                <div class="d-flex flex-wrap justify-content-end gap-3 mt-3">
                                    <button type="button" class="btn btn-secondary px-3"
                                        data-dismiss="modal">{{ translate('close') }}</button>
                                    <button type="button" class="btn btn--primary form-submit"
                                        data-form-id="submit-approve-form"
                                        data-message="{{ translate('want_to_approv_this_refund_request') . '?' }}"
                                        data-redirect-route="{{ route('vendor.refund.index', ['status' => $refund['status']]) }}">{{ translate('submit') }}</button>
                                </div>
                            </div>
                        </form>
        ...
        @endif
        <div class="modal fade" id="receivedModel">
            <div class="modal-dialog">
                <div class="modal-content">
                    <form action="{{ route('vendor.refund.update-status') }}" method="post" id="submit-received-form">
                        @csrf
                        <div class="modal-body">
                            <input type="hidden" name="id" value="{{ $refund->id }}">
                            <input type="hidden" name="refund_status" value="received">
                            <div class="text-center">
                                <img class="mb-3"
                                    src="{{ dynamicAsset(path: 'public/assets/back-end/img/received.png') }}"
                                    alt="{{ translate('product_received') }}" width="40px" height="40px">
                                <h4 class="mb-4 mx-auto max-w-283">
                                    {{ translate('you_can_received_that_refund_request_two_times') . ',' . translate('then_you_can’t_change_this_status') . '.' }}
                                </h4>
                            </div>
                            <textarea class="form-control text-area-max-min"
                                placeholder="{{ translate('please_write_the_received_reason') . '...' }}" name="received_note" rows="3"></textarea>
                            <div class="d-flex flex-wrap justify-content-end gap-3 mt-3">
                                <button type="button" class="btn btn-secondary px-3"
                                    data-dismiss="modal">{{ translate('close') }}</button>
                                <button type="button" class="btn btn--primary form-submit"
                                    data-form-id="submit-received-form"
                                    data-message="{{ translate('want_to_received_this_refund_request') . '?' }}"
                                    data-redirect-route="{{ route('vendor.refund.index', ['status' => $refund['status']]) }}">{{ translate('submit') }}</button>
                            </div>
                        </div>
                    </form>
