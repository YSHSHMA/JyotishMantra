    ...
    <div class="row">
        <div class="col-sm-6 col-lg-3 col-md-3  mt-2">
            <a class="order-stats order-stats_confirmed">
                ...
                <span class="order-stats__title"> {{ \App\Models\VendorSupportTicketConv::where('created_by','vendor')->where('type','seller')->where('vendor_id',auth('seller')->id())->whereNotNull('ticket_id')->count()}} </span>
            </a>
        </div>
        <div class="col-sm-6 col-lg-3 col-md-3  mt-2">
            <a class="order-stats order-stats_confirmed">
                ...
                <span class="order-stats__title">{{ \App\Models\VendorSupportTicketConv::where('created_by','vendor')->where('type','seller')->where('vendor_id',auth('seller')->id())->where('status','open')->count()}}</span>
                </span>
            </a>
        </div>
        <div class="col-sm-6 col-lg-3 col-md-3  mt-2">
            <a class="order-stats order-stats_confirmed">
                ...
                <span class="order-stats__title">{{ \App\Models\VendorSupportTicketConv::where('created_by','vendor')->where('type','seller')->where('vendor_id',auth('seller')->id())->where('status','close')->count()}} </span>
                </span>
            </a>
        </div>
    </div>
                            ...
                            <div
                                ...
                                <div class="text-nowrap">
                                    <a class="btn btn--primary"
                                        href="{{ route('vendor.support-ticket.vendor.singleTicket', $ticket['id']) }}">
    ...
    <div class="modal fade add-issues" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{route('vendor.support-ticket.vendor.store-inbox')}}" method="post">
