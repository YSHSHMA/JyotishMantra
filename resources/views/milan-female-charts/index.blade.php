<div id="femaleHoroscopeCharts" class="text-center"></div>
...
$(document).ready(function() {
    ...
    // Generate charts
    getFemaleCharts(options, type, 'femaleHoroscopeCharts', true);
    //console.log(north);

    // Adjust chart container size
    var $femaleHoroscopeCharts = $('#femaleHoroscopeCharts');
    var $svg = $femaleHoroscopeCharts.find('svg');

    if ($svg.length) {
        var chartWidth = parseInt($svg.attr('width'), 10);
        var chartHeight = parseInt($svg.attr('height'), 10);
        $femaleHoroscopeCharts.css({
            'width': chartWidth + 'px',
