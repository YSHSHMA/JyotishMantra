...
@section('content')
    <div class="content container-fluid">
        ...
        <div class="row">
            <div class="col-sm-12 col-lg-12 mb-3 mb-lg-2">
                <div class="card">
                    <div class="card-body">
                        <form action="{{ route('event-vendor.coupon.store-coupon') }}" method="post">
                            @csrf
                            <div class="row">
                                <div class="col-md-6 col-lg-4 form-group d-none">
                                    ...
                                    <select class="form-control" id="coupon_type" name="coupon_type" required>
                                        <option value="event_order" selected>{{translate('event_order')}}</option>
                                    </select>
                                </div>
                                ...
                                <input type="hidden" value="event" name="coupon_bearer">
                                ...
                                <div class="col-md-6 col-lg-4 form-group d-none">
                                    <label for="exampleFormControlInput1" class="title-color font-weight-medium d-flex">{{translate('limit_for_same_user')}}</label>
                                    <input type="number" name="limit" value="{{ old('limit',1) }}" min="0"
                        ...
                        <table id="datatable"
                            ...
                            @foreach($coupons as $k=>$coupon)
                                <tr>
                                    ...
                                    <td>
                                        @if($coupon->added_by=='seller' || ($coupon->added_by=='admin' && $coupon->coupon_bearer=='event' && $coupon->seller_id==$relationEmployees))
                                            <form action="{{route('event-vendor.coupon.update-status',['id'=>$coupon['id']])}}" method="get" id="coupon_status{{$coupon['id']}}-form" class="coupon_status_form">
                                        ...
                                        <div class="d-flex gap-10 justify-content-center">
                                            ...
                                            @if($coupon->added_by=='seller' || ($coupon->added_by=='admin' && $coupon->coupon_bearer=='event' && $coupon->seller_id==$relationEmployees))
                                                <a class="btn btn-outline--primary btn-sm edit"
                                                   href="{{route('event-vendor.coupon.coupon_update',[$coupon['id']])}}"
                                                ...
                                                <form action="{{route('event-vendor.coupon.delete',[$coupon['id']])}}" method="post" id="coupon-{{$coupon['id']}}">
                                                    @csrf @method('delete')
                                                </form>
                                            @else
                                                <button
    ...
    <span id="get-detail-url" data-url="{{ route('event-vendor.coupon.quick-view') }}"></span>
@endsection
@push('script')
