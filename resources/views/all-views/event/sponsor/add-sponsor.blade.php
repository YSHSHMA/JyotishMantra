...
@section('title', translate('Add_Sponsor'))
@php
...
<div class="content container-fluid">
    ...
    <div class="row">
        <div class="col-sm-12 col-lg-12 mb-3 mb-lg-2">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('event-vendor.sponsor.store-sponsor') }}" method="post" enctype="multipart/form-data">
                        @csrf
                        <div class="row">
                            <div class="col-md-6 col-lg-4 form-group d-none">
                                ...
                                <select name="type" class="form-control" value="{{ old('type') }}" required>
                                    ...
                                    <option value="sponsor" {{ ((old('type') == 'sponsor' ) ? "selected" : '' )}} selected>{{translate('Sponsor')}}</option>
                                    <option value="complimentary"{{ ((old('type') == 'complimentary' ) ? "selected" : '' )}}>{{translate('complimentary_pass')}}</option>
                                </select>
                            </div>
                            ...
                            <div class="col-md-6 col-lg-4 form-group">
                                ...
                                <input type="text" name="company_name" class="form-control" value="{{ old('company_name') }}" placeholder="{{translate('company_name')}}">
                            </div>

                            <div class="col-md-6 col-lg-4 form-group">
                                <label for="name" class="title-color font-weight-medium d-flex">{{translate('phone')}}</label>
                                <input class="form-control text-align-direction phone-input-with-country-picker" type="tel" value="" id="person-number" placeholder="{{ translate('enter_phone_number') }}" required {{ isset($customer['phone']) ? 'readonly' : '' }} oninput="this.value=this.value.slice(0,10)">
                                <input type="hidden" class="country-picker-phone-number w-50" value="" name="person_phone" readonly>
                            ...
                            <div class="col-md-6 col-lg-4 form-group">
                                ...
                                <input type="text" name="link" value="{{ old('link') }}" class="form-control" placeholder="{{translate('contact_link')}}">
                            </div>
                            <div class="col-md-6 col-lg-4 form-group">
                                <label for="name" class="title-color font-weight-medium d-flex">{{translate('Pass_list')}}</label>
                                <select name="package_id[]" value="{{ old('package_id') }}" class="form-control" required multiple>
                                    <option value="">{{translate('Package_Select')}}</option>
                                    @if($packageList)
                                    @foreach($packageList as $va)
                                    <option value="{{$va['id']}}">{{$va['package_name']}}</option>
                                    @endforeach
                                    @endif
                                </select>
                            </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div>
                                                <div class="custom_upload_input">
                                                    <input type="file" name="image" class="custom-upload-input-file action-upload-color-image image-preview-before-upload" id="" data-imgpreview="pre_img_viewer" accept=".jpg, .webp, .png, .jpeg, .gif, .bmp, .tif, .tiff|image/*" required>
