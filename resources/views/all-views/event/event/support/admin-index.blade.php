    ...
    <div class="row">
        <div class="col-sm-6 col-lg-3 col-md-3  mt-2">
            <a class="order-stats order-stats_confirmed">
                ...
                <span class="order-stats__title"> {{ \App\Models\VendorSupportTicketConv::where('created_by','admin')->where('vendor_id',$relationEmployees)->where('type','event')->whereNotNull('ticket_id')->count()}} </span>
            </a>
        </div>
        <div class="col-sm-6 col-lg-3 col-md-3  mt-2">
            <a class="order-stats order-stats_confirmed">
                ...
                <span class="order-stats__title">{{ \App\Models\VendorSupportTicketConv::where('created_by','admin')->where('vendor_id',$relationEmployees)->where('type','event')->where('status','open')->count()}}</span>
                </span>
            </a>
        </div>
        <div class="col-sm-6 col-lg-3 col-md-3  mt-2">
            <a class="order-stats order-stats_confirmed">
                ...
                <span class="order-stats__title">{{ \App\Models\VendorSupportTicketConv::where('created_by','admin')->where('vendor_id',$relationEmployees)->where('type','event')->where('status','close')->count()}} </span>
                </span>
            </a>
        </div>
    </div>
                    ...
                    <div class="card">
                        <div
                            ...
                            <div class="media gap-3">
                                ...
                                <img class="avatar avatar-lg" src="{{ getValidImage(path: 'storage/app/public/event/organizer/' . $ticket['Event']['image'] ?? '', type: 'backend-profile') }}" alt="">
                                ...
                                <form action="{{ route('event-vendor.messages.status') }}" method="post" id="support-ticket{{ $ticket['id'] }}-form">
                                    @csrf
                            ...
                            <div
                                ...
                                <div class="text-nowrap">
                                    @if (Helpers::Employee_modules_permission('Support Management', 'From Admin', 'Chat'))
                                    <a class="btn btn--primary" href="{{ route('event-vendor.message.singleTicket', $ticket['id']) }}">
