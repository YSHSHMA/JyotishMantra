        ...
        <div class="col-md-12 mb-2">
            <div class='row'>
                <div class="col-sm-6 col-lg-3 col-md-3  mt-2">
                    <a class="order-stats order-stats_confirmed" href="{{ route('event-vendor.event-management.event-complate')}}">
                        ...
                        <span class="order-stats__title">
                            @php
                            if(auth('event')->check()){
                            echo \App\Models\Events::where('event_organizer_id',auth('event')->user()->relation_id)->where('is_approve', 1)->where('status', 1)->whereRaw("
                            DATE(?) > STR_TO_DATE(
                            IF(INSTR(start_to_end_date, ' - ') > 0,
                            SUBSTRING_INDEX(start_to_end_date, ' - ', -1),
                            start_to_end_date
                            ), '%Y-%m-%d')
                            ", [now()->format('Y-m-d')])->count();
                            }elseif(auth('event_employee')->check()){
                            echo \App\Models\Events::where('event_organizer_id',auth('event_employee')->user()->relation_id)->where('is_approve', 1)->where('status', 1)->whereRaw("
                            DATE(?) > STR_TO_DATE(
                            IF(INSTR(start_to_end_date, ' - ') > 0,
                            SUBSTRING_INDEX(start_to_end_date, ' - ', -1),
                            start_to_end_date
                            ...
                            <thead class="thead-light thead-50 text-capitalize">
                                <tr>
                                    ...
                                    <th>{{ translate('Verification_status') }}</th>
                                     @if(Helpers::Employee_modules_permission('Event Management', 'Complete List', 'Edit'))
