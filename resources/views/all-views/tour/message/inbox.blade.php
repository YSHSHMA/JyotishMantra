            ...
            <div class="col-sm-6 col-lg-3 col-md-3  mt-2">
                <a class="order-stats order-stats_confirmed">
                    ...
                    <span class="order-stats__title">
                        {{ \App\Models\VendorSupportTicketConv::where('created_by', 'vendor')->where('vendor_id',auth('tour')->user()->relation_id??'')->where('type', 'tour')->whereNotNull('ticket_id')->count() }}
                    </span>
                </a>
            </div>
            <div class="col-sm-6 col-lg-3 col-md-3  mt-2">
                <a class="order-stats order-stats_confirmed">
                    ...
                    <span
                        class="order-stats__title">{{ \App\Models\VendorSupportTicketConv::where('created_by', 'vendor')->where('vendor_id',auth('tour')->user()->relation_id??'')->where('type', 'tour')->where('status', 'open')->count() }}</span>
                    </span>
                </a>
            </div>
            <div class="col-sm-6 col-lg-3 col-md-3  mt-2">
                <a class="order-stats order-stats_confirmed">
                    ...
                    <span
                        class="order-stats__title">{{ \App\Models\VendorSupportTicketConv::where('created_by', 'vendor')->where('vendor_id',auth('tour')->user()->relation_id??'')->where('type', 'tour')->where('status', 'close')->count() }}
                    </span>
                    </span>
                </a>
            </div>
                            ...
                            <div class="card">
                                <div
                                    ...
                                    <div class="media gap-3">
                                        ...
                                        @if ($ticket['Tour'] && $ticket['type'] == 'tour')
                                            <img class="avatar avatar-lg"
                                                src="{{ getValidImage(path: 'storage/app/public/tour_and_travels/doc/' . $ticket['Tour']['image'] ?? '', type: 'backend-profile') }}"
                                                alt="">
                                    ...
                                    @if ($ticket['status'] == 'open')
                                        <form action="{{ route('tour-vendor.messages.status') }}" method="post"
                                ...
                                <div
                                    ...
                                    <div class="text-nowrap">
                                        <a class="btn btn--primary"
                                            href="{{ route('tour-vendor.messages.singleTicket', $ticket['id']) }}">
    ...
    <div class="modal fade add-issues" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('tour-vendor.messages.store-inbox') }}" method="post">
