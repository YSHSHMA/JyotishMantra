    ...
    <div class="row">
        <div class="col-sm-6 col-lg-3 col-md-3  mt-2">
            <a class="order-stats order-stats_confirmed">
                ...
                <span class="order-stats__title"> {{ \App\Models\VendorSupportTicketConv::where('created_by','admin')->where('vendor_id',auth('tour')->user()->relation_id??'')->where('type','tour')->whereNotNull('ticket_id')->count()}} </span>
            </a>
        </div>
        <div class="col-sm-6 col-lg-3 col-md-3  mt-2">
            <a class="order-stats order-stats_confirmed">
                ...
                <span class="order-stats__title">{{ \App\Models\VendorSupportTicketConv::where('created_by','admin')->where('vendor_id',auth('tour')->user()->relation_id??'')->where('type','tour')->where('status','open')->count()}}</span>
                </span>
            </a>
        </div>
        <div class="col-sm-6 col-lg-3 col-md-3  mt-2">
            <a class="order-stats order-stats_confirmed">
                ...
                <span class="order-stats__title">{{ \App\Models\VendorSupportTicketConv::where('created_by','admin')->where('vendor_id',auth('tour')->user()->relation_id??'')->where('type','tour')->where('status','close')->count()}} </span>
                </span>
            </a>
        </div>
    </div>
                    ...
                    <div class="card">
                        <div
                            ...
                            <div class="media gap-3">
                                ...
                                <form action="{{ route('tour-vendor.messages.status') }}" method="post" id="support-ticket{{ $ticket['id'] }}-form">
                                    @csrf
                            ...
                            <div
                                ...
                                <div class="text-nowrap">
                                    <a class="btn btn--primary"
                                        href="{{ route('tour-vendor.message.singleTicket', $ticket['id']) }}">
