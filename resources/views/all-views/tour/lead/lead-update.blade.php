...
<div class="content container-fluid">
    ...
    <form class="product-form text-start" action="{{ route('tour-vendor.lead.tour-admin-lead-update',['id'=>$getData['id']]) }}" method="POST" enctype="multipart/form-data" id="services_form">
        @csrf
        <div class="card">
            <div class="card-body">
                <div class="">
                    ...
                    <div class="row mt-4">
                        <div class="col-md-4">
                            ...
                            <select name="platform" id="platform " class="form-control" required="">
                                ...
                                <option value="app" {{ (($getData['platform'] == 'app' )?"selected":"" ) }}>App</option>
                                <option value="web" {{ (($getData['platform'] == 'web' )?"selected":"" ) }}>Website</option>
                                <option value="admin" {{ (($getData['platform'] == 'admin' )?"selected":"" ) }}>Admin Side</option>
                                <option value="instagram" {{ (($getData['platform'] == 'instagram' )?"selected":"" ) }}>Instagram</option>
                                <option value="facebook" {{ (($getData['platform'] == 'facebook' )?"selected":"" ) }}>Facebook</option>
                                <option value="ads" {{ (($getData['platform'] == 'ads' )?"selected":"" ) }}>Ads</option>
                            </select>
                        </div>
                        <div class="col-md-4">
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <input type="text" value="{{ $getData['userData']['name']}}" required name="user_name" class="form-control" id="person-name" placeholder="{{ translate('user_name') }}">
                            </div>
                        </div>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select type="text" name="tour_id" class="form-control selectpicker" data-live-search="true">
                                    ...
                                    <option value="{{ $va['id'] }}" {{ (($getData['tour_id'] == $va['id'] )?"selected":"" ) }}>{{ $va['tour_name'] }} ({{ (($va['is_person_use'] == 1)?"Per Head":"Cab")}}) {{ $va['number_of_day'] . 'D/' . $va['number_of_night'] . 'N' }}</option>
                                    @endforeach
                                    @endif
                                </select>
                            </div>
            ...
            <div class="modal-body">
                    ...
                    <span>
                        <form class="needs-validation" action="javascript:" method="post" novalidate id="coupon-code-events-ajax">
                            <div class="d-flex form-control rounded-pill ps-3 p-1">
                                ...
                                <input type="hidden" name="user_id" value="{{ $getData['userData']['id']??''}}" class="get_customer_id">
                                ...
                                <button class="btn btn--primary rounded-pill text-uppercase py-1 fs-12 coupan_apply_text" type="button" id="events-coupon-code">
                                    {{ translate('apply') }}
                                </button>
                            </div>
                ...
                <div class="justify-content-between my-2">
                    <span>
                        ...
                        <input type="text" class="form-control w-50 amount_payment_display d-none">
                    </span>
                </div>
                ...
                <button onclick="Pay_nowUse()" class="btn btn-primary">pay</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    function pay_summary() {
        ...
        TourArrayBooking.forEach((item, i) => {
            ...
            <tr>
                <td>${i + 1}</td>
                <td>${((item.type == 'route') ? (item.type+' ('+item.price+')') : (item.title||item.type))}</td>
                <td>${item.qty}</td>
                <td>${Number(item.price || 0).toFixed(2)}</td>
                <td>${(item.gst||0)}%</td>
                <td>${Number(item.tax_price || 0).toFixed(2)}</td>
...
<script>
    $('.selectpicker').on('changed.bs.select', function(e, clickedIndex, isSelected, previousValue) {
        ...
        GetTourInfo(selectedValue);
    });
    GetTourInfo('{{ $getData["tour_id"]}}');

    function GetTourInfo(selectedValue) {
        ...
        $.ajax({
            url: "{{ route('tour-vendor.lead.get-tour-info-div') }}",
            type: "post",
            data: {
    ...
    function updateInformationOld() {
        if ("{{ $getData['tour_id'] }}" == $('.selectpicker').val()) {
            $('.hasDatepicker').val("{{ $getData['pickup_date'] }}");
            $('input[name="time"],select[name="time"]').val("{{ $getData['pickup_time'] }}");
            $('.pickup_lat').val("{{ $getData['pickup_lat'] }}");
            $('.pickup_long').val("{{ $getData['pickup_long'] }}");
            $('#pickup_location').val("{{ $getData['pickup_address'] }}");
            // TourArrayBooking = JSON.parse("{{ $getData['booking_package'] }}");
            let jsonString = @json($getData['booking_package'] ?? []);
            if (JSON.parse(jsonString)) {
                TourArrayBooking.splice(0, TourArrayBooking.length);
                console.log(TourArrayBooking);
                let newArray = JSON.parse(jsonString);
                TourArrayBooking.push(...newArray);
                if ($('.is_person_use_tour').val() == 1) {
                    let perPersons = TourArrayBooking.find(item => item.type === 'per_head');
                    let PerRoutes = TourArrayBooking.find(item => item.type === 'route');
                    let PerTransport = TourArrayBooking.find(item => item.type === "transport");
                    let input = document.querySelector('.per_head_' + perPersons.id);
                    if (input) {
                        input.value = perPersons.qty;
                    ...
                    if (PerRoutes && (PerRoutes.price == 'Both' || PerRoutes.price == 'Drop' || PerRoutes.price == 'Pickup')) {
                        $(`.extracharges-transport[data-type="${PerRoutes.price}"]`).click();
                    } else if (PerTransport) {
                        $(`.extracharges-transport[data-type="Both"]`).click();
                    }
                } else {
                    let perPersons = TourArrayBooking.find(item => item.type === 'cab');
                    let input = document.querySelector('.cab_id_' + perPersons.id);
                    if (input) {
                        input.value = perPersons.qty;
                        input.dispatchEvent(new Event('input', {
                            bubbles: true
                ...
                let grouped = TourArrayBooking.reduce((acc, item) => {
                    if (!acc[item.type]) {
                        acc[item.type] = [];
                    }
                ...
                let otherItems = grouped['other'] || [];
                let foodsItems = grouped['foods'] || [];
                let hotelItems = grouped['hotel'] || [];
                let allItems = [...otherItems, ...foodsItems, ...hotelItems];
                allItems.forEach(perPersons => {
                    let input = document.querySelector('.other_packages_' + perPersons.id);
                    if (input) {
                        input.value = perPersons.qty;
                        input.dispatchEvent(new Event('input', {
                            bubbles: true
...
<script>
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (existingRoute !== -1) {
                        ...
                        if (TourArrayBooking[existingRoute].price > 0) {
                            TourArrayBooking[existingRoute].price = (TourArrayBooking[existingRoute].price || 0) + response.ExChargeAmount;
                        }
                    }
                    if (TourArrayBooking[existingRoute] && TourArrayBooking[existingRoute].price !== undefined) {
                        $('.getAddress_google').data('ex-charge-driver', TourArrayBooking[existingRoute].price);
                    } else {
    ...
    function calculateDistance() {
        ...
        if (lat2 && lng2) {
            ...
            if (existingItem) {
                ...
                existingItem.total_price = (Math.ceil(parseFloat(additionalCharge) * 100) / 100) || 0;
            } else {
                TourArrayBooking.push({
                    type: 'ex_distance',
                    id: '0',
                    ...
                    title: "Ex Distance",
                    tax_price: 0,
                    total_price: (Math.ceil(parseFloat(additionalCharge) * 100) / 100) || 0,
                });
            }
    ...
    function transportOption(that = null) {
        ...
        if (transItem) {
            ...
            transItem.total_price = pricetrans,
