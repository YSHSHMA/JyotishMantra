...
if (auth('tour')->check()) {
$relationId = auth('tour')->user()->relation_id;
} elseif (auth('tour_employee')->check()) {
$relationId = auth('tour_employee')->user()->relation_id;
} else {
$relationId = null;
}
                ...
                <div class="px-3 py-4">
                    <div class="row g-2 flex-grow-1 my-2">
                        <div class="col-sm-12 col-md-12 col-lg-12">
                            ...
                            <a href="{{ route('tour-vendor.lead.add-lead') }}" target="_blank" rel="noopener noreferrer" class="btn btn-primary float-end">Add Lead</a>
                            @endif
                        </div>
                        <div class="col-sm-12 col-md-12 col-lg-12">
                            <?php $getHeadCount = \App\Models\TourLeads::where('create_by_vendor',$relationId); ?>
                            <div class="row">
                                <div class="col-md-4">
                                ...
                                <div class="col-md-4">
                                    <div class="card-col-5 mb-3">
                                        <div class="card shadow-sm h-100 border-0 rounded-lg">
                                            <h6 class="card-body text-capitalize font-weight-bold mb-3 justify-content-between d-flex">
                                                <span class="badge badge-warning px-4 py-3" style="font-size: 14px;">Pending</span>
                                                <div>
                                                    <span style="font-size: 29px;">{{ (clone $getHeadCount)->where('amount_status',0)->count() }}</span>
                                                </div>
                                            </h6>
                                        </div>
                                    </div>
...
<script>
    ...
    function followHistory(that) {
        ...
        var row = "";
        $.ajax({
            url: "{{ url('tour-vendor/lead/tour-follow-up') }}/" + leadId,
            method: 'GET',
            //   data:{id:leadId},
            success: function(response) {
...
<script>
    $(document).ready(function() {
        initDataTable({
            ...
            ajaxUrl: "{{ route('tour-vendor.lead.lead-list-filter') }}",
