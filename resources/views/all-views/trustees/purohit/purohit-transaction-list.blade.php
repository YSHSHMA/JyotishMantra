                                                ...
                                                @foreach ($transactions as $key => $item)
                                                    <tr>
                                                        ...
                                                        <td class="d-flex align-items-center justify-content-between" style="min-width: 200px;">
                                                            <div>
                                                                <strong>Order Date:{{ date('d-m-Y h:i A',strtotime($item->templeOrder->created_at??"")) }},</strong><br>
                                                                <strong>Schedule Date:{{ date('d-m-Y',strtotime($item->templeOrder->booking_date??"")) }} @if($item['templeOrder']['time_slot']??'' != 'pending') {{ date('h:i s',strtotime(explode(' - ',$item['templeOrder']['time_slot']??"")[0])) }} @endif,</strong><br>
                                                                <small>Booking  Id:{{ $item->order_id }},</small><br>
                                                                <small>Package Price:{{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $item->package_price ?? 0), currencyCode: getCurrencyCode()) }},</small><br>
                                                                <small>Package Name:{{ $item->package->varient_name ?? '-' }}</small>
                                                            </div>
                                                        </td>
                                                        <td>
