                ...
                <div class="card h-100">
                    <div class="card-body">
                        ...
                        <div class="d-flex flex-column gap-10">
                            ...
                            <span class="d-flex align-items-center gap-10 text-capitalize">
                                <img
                                ...
                                <div class="d-flex flex-column gap-2 mt-3">
                                    ...
                                    <div class="payment-method d-flex justify-content-sm-end gap-10 text-capitalize">
                                        ...
                                        @if($details->payment_status == 1)
                                            <strong class="text-success">{{ translate('Paid') }}</strong>
                                        @elseif($details->payment_status == 0)
                                            <strong class="text-warning">{{ translate('Unpaid') }}</strong>
                                        @elseif($details->payment_status == 2)
                                            <strong class="text-danger">{{ translate('Cancel') }}</strong>
                                        @endif
                                    </div>
                          ...
                          <span class="title-color"><i class="tio-tv-new"></i>
                            ...
                            @php
                              // Check if a valid YouTube URL exists
                                $youtubeLink = isset($details['live_stream']) && !empty($details['live_stream'])  ? $details['live_stream']  : 'https://www.youtube.com/';
    ...
    <div class="modal fade" id="order-time-modal" tabindex="-1" role="dialog" aria-labelledby="modelTitleId"
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <div>
                        <h5 class="modal-title">{{ translate('Pooja Time Schedule')}}</h5>
                        <p class="modal-subtitle" style="font-size: 14px; color: #555; margin: 5px 0 0;">
                        {{ translate('Below is the schedule for the  pooja timings. Please ensure to arrive at least 15 minutes before the scheduled time.') }}
                        </p>
                    </div>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                ...
                <form action="{{ route('guruji.orders.puja.status_time', ['id' => $vendorId]) }}" method="post" id="pooja-time-form" class="modal-form">
                    @csrf
                    <div class="modal-body">
                        <input type="hidden" name="booking_date" id="booking_id" value="{{ $details->booking_date }}">
                        <input type="hidden" name="service_id" id="service-id" value="{{ $details->service_id }}">
                        <input type="hidden" name="pandit_id" id="pandit-id" value="{{ $details->pandit_assign }}">
                        <input type="hidden" name="order_status" id="order-time-status">
                        <input type="text" name="schedule_time" id="pooja_time" placehoder="{{ translate('Schedule Time') }}" class="schedule-time form-control"
