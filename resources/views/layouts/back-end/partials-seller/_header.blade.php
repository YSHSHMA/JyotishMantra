                    ...
                    <li class="nav-item">
                        <div class="hs-unfold">
                            <a
                                ...
                                @php($notification=App\Models\Notification::whereBetween('created_at', [auth('seller')->user()->created_at, Carbon::now()])->where('sent_to', 'seller')->where('status', '1')->whereDoesntHave('notificationSeenBy')->count())
                                @php($notification2=App\Models\Notification::whereBetween('created_at', [auth('seller')->user()->created_at, Carbon::now()])->where('sent_to', 'seller')->where('sent_by',auth('seller')->id())->where('status', '0')->whereDoesntHave('notificationSeenBy')->count())
                                @if($notification != 0 || $notification2 != 0 )
                                <span class="btn-status btn-sm-status btn-status-danger notification_data_new_count">{{ ($notification + $notification2) }}</span>
                                @endif
                            </a>
                            <div id="notificationDropdown"
                                ...
                                @php($notification_data=\App\Models\Notification::whereBetween('created_at', [auth('seller')->user()->created_at, Carbon::now()])->where('sent_to', 'seller')->where('status', '1')->with('notificationSeenBy')->latest()->get())
                                @php($notification_data2=\App\Models\Notification::whereBetween('created_at', [auth('seller')->user()->created_at, Carbon::now()])->where('sent_to', 'seller')->where('sent_by',auth('seller')->id())->where('status', '0')->with('notificationSeenBy')->latest()->get())
