        ...
        <div class="navbar-nav-wrap">
            ...
            <div class="navbar-nav-wrap-content-right"
                ...
                <ul class="navbar-nav align-items-center flex-row gap-xl-16px">                    
                    <li class="nav-item">
                        <div class="hs-unfold">
                            <a href="{{ route('guruji.individual', ['name' => Str::slug(auth('guruji')->user()->name)]) }}" title="{{translate('website_Tour_view')}}"
                        ...
                        <div class="hs-unfold">
                            <a class="js-hs-unfold-invoker media align-items-center gap-3 navbar-dropdown-account-wrapper dropdown-toggle dropdown-toggle-left-arrow"
                                ...
                                <div class="d-none d-md-block media-body text-right">
                                    @if(auth('guruji')->check())
                                        <h5 class="profile-name mb-0">{{ auth('guruji')->user()->name }}</h5>
                                    @endif
                                </div>
                                <?php
                                    $shop = \App\Models\Astrologer\Astrologer::where('id', ($relationEmployees ?? 0))->first();
                                    $logoImage =  getValidImage(path: 'storage/app/public/astrologer' . ($shop['image'] ?? ''), type: 'backend-logo');
                                ?>
                                @php($shop = \App\Models\Astrologer\Astrologer::where('id', ($relationEmployees ?? 0))->first())
                            ...
                            <div id="accountNavbarDropdown"
                                ...
                                <div class="dropdown-item-text">
                                    <div class="media align-items-center text-break">
                                        <div class="avatar avatar-sm avatar-circle mr-2">
                                            <img class="avatar-img"
                                                src="{{ getValidImage(path: 'storage/app/public/astrologers' . ($shop['image'] ?? ''), type: 'backend-logo') }}"
                                                alt="{{translate('image_description')}}">
                                        </div>
                                        <div class="media-body">
                                            @if(auth('guruji')->check())
                                            <span class="card-title h5">{{auth('guruji')->user()->name}}</span>
                                            <span class="card-text">{{auth('guruji')->user()->email}}</span>
                                ...
                                <a class="dropdown-item" href="{{ route('guruji.profile.update', auth('guruji')->user()->id) }}">
