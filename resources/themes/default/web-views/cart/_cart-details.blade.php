...
@php(
    $cart = \App\Models\Cart::where(['customer_id' => auth('customer')->check() ? auth('customer')->id() : session('guest_id')])->get()->groupBy('cart_group_id')
)
@php( $showShippingSelect = false)
            ...
            <table
                ...
                <thead class="thead-light">
                    <tr class="">
                        ...
                        <th class="font-weight-bold pl-0 __w-15p text-capitalize">{{ translate('main_price') }}</th>
                        <th class="font-weight-bold pl-0 __w-15p text-capitalize">{{ translate('discounted_price') }}</th>
                            ...
                            @foreach ($group as $cart_key => $cartItem)
                                ...
                                <tr>
                                    <td class="__w-45">
                                        <div class="d-flex gap-3">
                                            ...
                                            <div class="d-flex flex-column gap-1">
                                                <div
                                                    class="text-break __line-2 {{ $checkProductStatus == 0 ? 'custom-cart-opacity-50' : '' }}">
                                                    <a
                                                    ...
                                                    @foreach (json_decode($cartItem['variations'], true) as $key1 => $variation)
                                                        <div class="">
                                                            <span class="__text-12px text-capitalize">
                                                                <span class="text-muted">{{ $key1 }} </span> :
                                    ...
                                    <td>
                                        ...
                                        ({{ translate('discount') }}
                                        :
                                        {{ webCurrencyConverter(amount: $cartItem['discount']) }})
                                        </span>
                                    </td>
                                        ...
                                        @if ($checkProductStatus == 1)
                                            <div class="qty d-flex justify-content-center align-items-center gap-3">
                                                <span class="qty_minus action-update-cart-quantity-list"
                                                    ...
                                                    <i
                                                        class="{{ $cartItem['quantity'] == (isset($cartItem->product->minimum_order_qty) ? $cartItem->product->minimum_order_qty : 1) ? 'tio-delete text-danger' : 'tio-remove' }}"></i>
                                                </span>
                                                <input type="text"
                    ...
                    @if ($free_delivery_status['status'] && (session()->missing('coupon_type') || session('coupon_type') != 'free_delivery'))
                        <div class="free-delivery-area px-3 mb-3 mb-lg-2">
                            <div class="d-flex align-items-center gap-8">
                                <img class="__w-30px" src="{{ theme_asset('public/assets/front-end/img/icons/free-shipping.png') }}" alt="">
                                @if ($free_delivery_status['amount_need'] <= 0)
                                    <span class="text-muted fs-12 mt-1">{{ translate('you_Get_Free_Delivery_Bonus') }}</span>
                                @else
                                    @php($showShippingSelect = true)
                                    <span class="need-for-free-delivery font-bold fs-12 mt-1 text-primary">{{ webCurrencyConverter($free_delivery_status['amount_need']) }}</span>
                            ...
                            @if ($showShippingSelect)
                                ...
                                <script>
                                   $(document).ready(function(){
                                    function updateshippingjjh(){
                                        $.ajax({
                                            type: "GET",
                                            url: "{{url('customer/set-shipping-method')}}", 
                                            data: "id=9&cart_group_id=all_cart_group",
                                    ...
                                    @if (!empty($chosenShipping))
                                        <input type="hidden" name="shipping_method_id" value="{{ $chosenShipping['shipping_method_id'] }}">
                                        <input type="hidden" name="shipping_method_title" value="{{ $shippings->firstWhere('id', $chosenShipping['shipping_method_id'])['title'] ?? '' }}">
                                        <input type="hidden" name="shipping_method_duration" value="{{ $shippings->firstWhere('id', $chosenShipping['shipping_method_id'])['duration'] ?? '' }}">
                                        <input type="hidden" name="shipping_method_cost" value="{{ $shippings->firstWhere('id', $chosenShipping['shipping_method_id'])['cost'] ?? '' }}">
                                    @endif
                                @endif
                        </div>
                    </div>
