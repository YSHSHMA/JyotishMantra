@if ($digital_payment['status'] == 1)
    @foreach ($payment_gateways_list as $payment_gateway)
        <form method="post" class="digital_payment" id="{{ $payment_gateway->key_name }}_form"
            action="{{ route('customer.offlinepooja-pending-payment-request') }}">
            @csrf
            ...
            <div class="Details">
                ...
                <input type="hidden" name="external_redirect_link"
                    value="{{ url('/') . '/offlinepooja-pending-web-payment' }}">
                ...
                <input type="hidden" name="order_id" value="{{ $poojaOrder->order_id }}">

            </div>
            <div class="mt-4">
                <button type="submit" class="btn badge badge-primary">Pay</button>
            </div>
