...
@section('title', translate('darshan_order_list'))
            ...
            <div class="card __card d-none d-lg-flex web-direction customer-profile-orders">
                <div class="card-body">
                    <div class="d-flex align-items-center justify-content-between gap-2 mb-0 mb-md-3">
                        <h5 class="font-bold mb-0 fs-16">{{ translate('darshan_order_list') }}</h5>
                    </div>

                    @if ($darshanOrders->count() > 0)
                    <div class="table-responsive">
                        <table class="table __table __table-2 text-center">
                            ...
                            <tbody>
                                @foreach ($darshanOrders as $donate)
                                <tr>
                                    <td>{{ $loop->iteration }}</td>
                                    <td class="bodytr">
                                        <div class="media-order">
                                            <a class="d-block position-relative">
                                                <img src="{{ getValidImage(path: 'storage/app/public/temple/thumbnail/' . ($donate['Temple']['thumbnail'] ?? ''), type: 'backend-logo') }}" alt="{{ translate('darshan') }}">
                                            ...
                                            <div class="cont text-start">
                                                ...
                                                <span class="fs-12 font-weight-medium">
                                                    {{ translate('person') }} : {{ ($donate['people_qty'] ?? '') }}
                                                </span><br>
                                    ...
                                    <td>
                                        <span>
                                            @if($donate['price'] > 0)
                                            {{ webCurrencyConverter(amount: $donate['price'] ?? 0) }}
                                            @else 
                                             {{ translate('Free Pass') }}
                                            @endif
                                        </span>
                                    ...
                                    <td>
                                        <a href="{{ route('darshan-order-details', [$donate['id']]) }}"
                                            title="view" class="btn-outline-success text-success __action-btn btn-shadow rounded-full">
                                            <i class="fa fa-eye"></i>
                                        </a>
                                        <a href="{{ url('api/v1/darshan/vip-invoice', [$donate['id']]) }}"
                    ...
                    <div class="text-center pt-5 text-capitalize">
                        <img src="{{ theme_asset(path: 'public/assets/front-end/img/sorry.svg') }}" alt=""
                            width="70">
                        <h5 class="mt-1 fs-14">{{ translate('no_vip_darshan_order_found') }}!</h5>
                    </div>
                    @endif
                ...
                <div class="card-body d-flex flex-column gap-3 customer-profile-orders py-0">
                    ...
                    <div class="d-flex border-lighter rounded p-2 justify-content-between gap-1">
                        ...
                        <div class="__btn-grp-sm">
                            <a href="{{ route('darshan-order-details', [$donate['id']]) }}" title="view" class="btn-outline-success text-success __action-btn btn-shadow rounded-full">
                                <i class="fa fa-eye"></i>
                            </a>
                            <a href="{{ url('api/v1/darshan/vip-invoice', [$donate['id']]) }}" title="Download Vip Darshan invoice" class="btn-outline-success text-success __action-btn btn-shadow rounded-full">
                                <i class="tio-download-to"></i>
                            </a>
                        </div>
                    </div>
