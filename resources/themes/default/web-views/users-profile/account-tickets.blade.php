                ...
                <form class="mt-3" method="post" action="{{route('ticket-submit')}}" id="open-ticket" enctype="multipart/form-data">
                    ...
                    <div class="form-row">
                        <div class="form-group col-md-6">
                            <label class="font-label">{{translate('type')}}</label>
                            <select class="custom-select" id="ticket_type_id" name="ticket_type_id" required>
                                <option value="">{{translate('Select_query')}}</option>
                                @if($TicketType)
                                @foreach($TicketType as $t_val)
                        ...
                        <div class="form-group col-md-6">
                            <label class="font-label">{{translate('issue')}}</label>
                            <select class="custom-select" id="ticket_issue_id" name="ticket_issue_id" required>
                                <option value="">{{translate('Select_Issue')}}</option>
                            </select>
                        </div>
                        <div class="form-group col-md-6">
                        ...
                        <div class="table-responsive">
                            <table class="table mb-0 __table-2 fs-13">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td> <span class="marl fs-13 font-semibold">{{($ticket['TicketType']['name']??'')}}</span></td>
                                        <td> <span class="marl fs-13 font-semibold">{{($ticket['TicketIssue']['issue_name']??'')}}</span></td>
                                        <td>
                                        ...
                                        <td>
                                            <div class="btn--container flex-nowrap justify-content-center">
                                                @php
                                                $getCountunread = \App\Models\SupportTicketConv::where('support_ticket_id',$ticket['id'])->where('read_user_status',0)->count();
                                                @endphp
                                                @if($getCountunread > 0)
                                                <a class="__action-btn btn-shadow rounded-full text-success" data-toggle="tooltip" data-title="New Message">{{ $getCountunread}}</a>
                                                @endif
...
<script>
    $(document).ready(function() {
        var ticketIssues = @json($TicketIssue);
        $('#ticket_type_id').on('change', function() {
            ...
            $ticketIssueDropdown.append('<option value="">{{ translate("Select_Issue") }}</option>');

            if (ticketTypeId) {
                const filteredIssues = ticketIssues.filter(function(issue) {
                    return issue.type_id == ticketTypeId;
                });
                filteredIssues.forEach(function(issue) {
                    $ticketIssueDropdown.append(
