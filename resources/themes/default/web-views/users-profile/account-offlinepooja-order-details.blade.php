            ...
            <section class="col-lg-9">
                ...
                <div class="bg-white border-lg rounded mobile-full">
                    <div class="p-lg-3 p-0">
                                                    ...
                                                    <div class="">
                                                        ...
                                                        <div class="fs-12">
                                                            ...
                                                            {{-- @if ($order['payment_status'] == 1) --}}
                                                                <span
                                                                    class="text-success text-capitalize">{{ translate($order['remain_amount_status']==0?'partially_paid':'full_paid') }}</span>
                                                            {{-- @endif --}}
                                                        </div>
                                                        ...
                                                        <div class="mt-2 fs-12">
                                                            <span
                                                                class="text-muted text-capitalize">{{ translate('order_date') }}</span>
                                                            :<span
                                                                class="text-primary text-capitalize">{{ date('d M, Y h:i A', strtotime($order['created_at'])) }}</span>
                                                        </div>
                                                    </div>
                                                </td>
                                        ...
                                        <tbody>
                                            <tr>
                                                <td>
                                                    <div class="media align-items-center gap-5">
                                                        <img class="d-block get-view-by-onclick rounded"
                                                            src="{{ getValidImage(path: 'storage/app/public/offlinepooja/thumbnail/' . $order['offlinePooja']['thumbnail'], type: 'backend-product') }}"
                                                            alt="{{ translate('image_Description') }}" style="width:50px">
                                                        <div class="ml-2">
                                                            <h6 class="title-color">
                                                                {{ Str::words($order['offlinePooja']['name'], 20, '...') }}
                                                ...
                                                <td>
                                                    ...
                                                    <div class="">
                                                        <span
                                                            class="fs-15 font-semibold">{{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $order['package_main_price']), currencyCode: getCurrencyCode()) }}</span>
                                                    </div>
                                                </td>
                                            </tr>
                                        </tbody>
                        ...
                        @if ($order['schedule_status']==1 && $order['status']==0)
                            <p class="text-success m-2">Your Pooja has been scheduled to <strong>{{date('d F Y', strtotime($order['booking_date']))}}</strong> with amount ₹{{$order['schedule_amount']}}</p>
                        @endif
                        @if ($order['status']==2 && $order['refund_status']==0)
                            <p class="text-danger m-2">Your Pooja has been canceled and your amount <strong>₹{{$order['refund_amount']}}</strong> will be refunded shortly</p>
                        @elseif ($order['status']==2 && $order['refund_status']==1)
                        <p class="text-success m-2">Your Pooja has been canceled and your amount <strong>₹{{$order['refund_amount']}}</strong> has been refunded to your wallet</p>
                        @endif
                        <div class="row d-flex justify-content-end mt-2">
                            <div class="col-md-8 col-lg-5">
                                                    ...
                                                    <td>
                                                        <div class="text-end">
                                                            <span class="font-weight-bold amount">
                                                                {{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $order->remain_amount), currencyCode: getCurrencyCode()) }}
                                                            </span>
                                                        </div>
                                                    </td>
                                                </tr>
                                ...
                                <div class="important-note">
                                    <strong>Important Note:</strong> Amount has been deducted via wallet for pooja advance booking. 
                                    @if ($order->remain_amount_status == 0)
                                        Remaining amount of {{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $order->remain_amount), currencyCode: getCurrencyCode()) }} will be paid at the time of pooja.
                                    @endif
                                </div>
                            @else
                                <div class="important-note">
                                    ...
                                    and wallet balance of 
                                    {{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $order->wallet_amount), currencyCode: getCurrencyCode()) }} 
                                     has been deducted for pooja advance booking.
                                     @if ($order->remain_amount_status == 0)
                                        Remaining amount of {{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $order->remain_amount), currencyCode: getCurrencyCode()) }} will be paid at the time of pooja.
                                    @endif
                                </div>
