                                            ...
                                            <div class="grid grid-cols-3 w-full mb-2 gap-4"
                                                ...
                                                @foreach ($packageIds as $key => $pac)
                                                    ...
                                                    <label for="tab-{{ $package->id }}"
                                                        ...
                                                        data-route="{{ route('poojastore', $epooja->slug) }}"
                                                        data-booking-date="{{ $epooja->pooja_type == '0' ? date('Y-m-d', strtotime($nextDate)) : date('Y-m-d', strtotime($earliestDate)) }}"
...
@push('script')
    ...
    <script>
        ...
        const serviceId = "{{ $epooja->id }}";
        const productId = "{{ $epooja->product_id }}";
        const route = "{{ route('poojastore', $epooja->slug) }}";
        const bookingDate =
            "{{ $epooja->pooja_type == '0' ? date('Y-m-d', strtotime($nextDate)) : date('Y-m-d', strtotime($earliestDate)) }}";
            ...
            function updatePackageContent(el) {
                ...
                if (authCheck == "true") {
                    ...
                    participationSection = `
                        <form class="needs-validation_" id="QueryForm" name="QueryForm" action="${route}" method="GET">
                            @csrf
