        ...
        <div class="container">
            <div class="row all-text-white">
                <div class="col-md-12 align-self-center">
                    <h1 class="innerpage-title">{{ ucwords(translate('Pooja_Book')) }}</h1>
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item active"><a href="{{ url('/') }}" class="text-white"><i class="fa fa-home"></i> Home</a></li>
                            <li class="breadcrumb-item">{{ ucwords(translate('Pooja_Book')) }}</li>
                        </ol>
                    </nav>
                </div>
            </div>
                                                    ...
                                                    <div class='d-flex flex-wrap gap-2 '>
                                                        <div class='text-center'>
                                                            <div class='fw-semibold' id='productPrice'>
                                                                {{ $pval->qty  }}  X  {{ webCurrencyConverter(amount: $pval->product_price *  $pval->qty) }}</div>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                    ...
                    <div class="__cart-total __cart-total_sticky">
                        <div class="cart_total p-0">
                                    ...
                                    <div class="pt-2">
                                        <div class="d-flex align-items-center form-control rounded-pill pl-3 p-1">
                                            ...
                                            <div class="px-2 d-flex justify-content-between w-100">
                                                ...
                                                <div class="bg-transparent text-danger cursor-pointer px-2 get-view-by-onclick-remove" data-link="{{ route('coupon.poojaremovecoupon', ['code' => session('coupon_code_pooja')]) }}">x</div>
                                            </div>
                                        </div>
                                    </div>
                                @else
                            ...
                            <div class="d-flex justify-content-between">
                                ...
                                <span class="cart_value" id="mainProductPrice"> {{ webCurrencyConverter(amount:  $leadsGet['package_price'] + $final_price_val - $couponDiscount) }}</span>
                            </div>
                            @else
                            <div class="d-flex justify-content-between">
                                <span class="cart_title text-primary font-weight-bold">{{translate('Total')}}</span>
                                <span class="cart_value" id="mainProductPrice"> {{ webCurrencyConverter(amount:  $leadsGet['package_price'] + $final_price_val) }}</span>
                            </div>
                            @endif
                        </div>
                        <hr class="my-2">
                            ...
                            @foreach ($payment_gateways_list as $payment_gateway)
                                <form method="post" class="digital_payment"  id="{{ $payment_gateway->key_name }}_form"
                                    ...
                                    <div class="Details">
                                        ...
                                        @if(session()->has('coupon_discount_pooja'))
                                        <input type="hidden" name="payment_amount" id="mainProductPriceInput" value="{{ $leadsGet['package_price'] + $final_price_val  - $couponDiscount }}">
                                        @else
                                        <input type="hidden" name="payment_amount" id="mainProductPriceInput" value="{{ $leadsGet['package_price'] + $final_price_val }}">
                                        @endif
                                    </div>
                                    <div class="mt-4">
                                        <button type="submit" class="btn btn--primary btn-block" data-id="{{ $leadsGet->id }}" data-name="{{ $leadsGet->service_name }}" data-price="{{ $leadsGet['package_name'] }}">{{translate('Proceed_to_Checkout')}}</button>
                                    </div>
                                </form>
                            @endforeach
                        @endif
                                    ...
                                    <div class="flash_deal_product_details pl-3 pr-3 pr-1 d-flex align-items-center">
                                        <div>
                                            ...
                                            <div class="widget-meta d-flex flex-wrap gap-8 align-items-center row-gap-0">
                                                {!! substr($product->details, 0, 40) !!}{!! strlen($product->details) > 10 ? '...' : '' !!}
                                            </div>
