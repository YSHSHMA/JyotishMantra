@php
    $coupon_dis = 0;
@endphp
@if (session()->has('coupon_discount_instancevip'))
    @php
        $couponDiscount = session()->has('coupon_discount_instancevip') ? session('coupon_discount_instancevip') : 0;
    @endphp
    <div class="d-flex justify-content-between">
        <span class="cart_title">{{ translate('coupon_discount_instancevip') }}</span>
    ...
    <div class="pt-2">
        <div class="d-flex align-items-center form-control rounded-pill pl-3 p-1">
            ...
            <div class="px-2 d-flex justify-content-between w-100">
                <div>
                    {{ session('coupon_code_instancevip') }}
                ...
                <div class="bg-transparent text-danger cursor-pointer px-2 get-view-by-onclick"
                    data-link="{{ route('coupon.poojaremovecoupon', ['code' => session('coupon_discount_instancevip')]) }}">
                    x</div>
            </div>
        </div>
    </div>
...
{{-- <div class="row"> --}}
        ...
        <form class="needs-validation" action="javascript:" method="post" novalidate id="coupon-code-pooja-ajax">
            <div class="d-flex form-control rounded-pill ps-3 p-1">
                <img width="24" src="{{ theme_asset(path: 'public/assets/front-end/img/icons/coupon.svg') }}"
                    alt="" onclick="instanceVipCouponList()">
    ...
    {{-- <div class="col-md-1 pt-2">
        <button class="btn btn--primary btn-sm rounded-pill" onclick="instanceVipCouponList()"><i
    ...
    @foreach ($payment_gateways_list as $payment_gateway)
        <form method="post" class="digital_payment" id="{{ $payment_gateway->key_name }}_form"
            ...
            <div class="Details">
                ...
                @endif
                <input type="hidden" name="external_redirect_link" value="{{ url('/') . '/vip-web-payment' }}">
                ...
                <input type="hidden" name="final_amount" id="Amount"  value="{{ $leadsGet['package_price'] + $final_price_val }}">
                @if (session()->has('coupon_code_instancevip'))
                    @php
                    $couponTotalAmount=$leadsGet['package_price'] + $final_price_val - $couponDiscount - $customer->wallet_balance;                                        
                    @endphp
                    @if($customer->wallet_balance < $productTotalAmount)
                        <input type="hidden" name="payment_amount" id="mainProductPriceInput" value="{{$couponTotalAmount }}">
                    @else
                        <input type="hidden" name="payment_amount" id="mainProductPriceInput" value="{{ 0.00 }}"> 
                    @endif
                @else
                    @if($customer->wallet_balance < $productTotalAmount)
                        <input type="hidden" name="payment_amount" id="mainProductPriceInput" value="{{ $leadsGet['package_price'] + $final_price_val - $customer->wallet_balance }}">
                            @else
                        <input type="hidden" name="payment_amount" id="mainProductPriceInput" value="{{ 0.00 }}">
                    @endif
                @endif
