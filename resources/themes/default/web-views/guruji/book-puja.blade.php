...
<body>
    ...
    <main style="margin-top:100px;">
        ...
        <section class="container mb-3">
            <div class="card-hero">
                ...
                <span class="text-12 font-bold  line-clamp-2 text-ellipsis mb-0" style="color:#fe9802;">{{ strtoupper($puja->pooja_heading) }}</span>
                @endif
                ...
                <div class="flex flex-col">
                    <div class="flex items-center space-x-1 pt-2">
                        <div class="d-flex">
                            <img src="{{ theme_asset(path: 'public/assets/front-end/img/track-order/temple.png') }}" alt="Puja Venue" style="width:24px;height:24px;">
        ...
        <section class="container mb-2">
            <div class="row g-3 " id="packageList">
                ...
                <h4 class="mt-4 fw-bold text-start text-md-left text-warning">{{ translate('Select Your Puja Package') }}</h4>
                <p class="mb-1 text-muted text-start text-md-left">{{ translate('Select_the_best_puja_package_with_clear_pricing_and_services.') }}</p>
                ...
                <div class="col-6 col-md-6 col-lg-3">
                    <div class="package {{ $loop->first ? 'selected' : '' }}" style="--pkg-color: #fd891c;background:{{ $color }} ;"
                        data-id="{{ $package->id }}" data-name="{{ $package->title }}" data-price="{{ $pac->price ?? 0 }}" data-type="{{ $package->type }}"
                        data-pandit="{{ $pac->pandit_id }}">

                        <span class="person-badge" style="background: {{ $color }}">{{ $package->person }} Person</span>
                        <span class="select-circle"></span>

                        <h5 class="mt-4">{{ $package->title }}</h5>
                        <p class="text-secondary small">{{ $package->short_info ?? '' }}</p>
                        <div class="package-footer d-flex align-items-center justify-content-between mt-2" style="background: linear-gradient(to right, #eb181824, {{ $color }}); display: flex; align-items: center; justify-content: space-between; padding: 8px 12px; border: none; width: 100%; border-radius: 8px; cursor: pointer;">
                            <div class="price" style="color:#fff;">₹ {{ $pac->price ?? 0 }}/-</div>
                            <img src="{{ getValidImage(path: 'storage/app/logo/' . $package->image, type: 'product') ?? asset('default-image.png') }}" alt="Package" class="package-img">
                        </div>
                    </div>
                </div>
                @endif
        ...
        <section class="container mb-5">
            <div class="row g-2" id="productList">
                ...
                <div class="col-12 text-left">
                    <h4 class="mt-4 fw-bold text-start text-md-left text-warning">{{ translate('Offer Your Devotion') }}</h4>
                    <p class="text-muted">{{ translate('Along_with_your_Puja_,_you_may_offer_Chadhava_or_make_a_donation.') }}</p>
                </div>
                <div class="owl-carousel owl-theme" id="productCarousel">
                    ...
                    @if ($product)
                    <div class="item">
                        <div class="product-card border rounded p-2 h-100 d-flex flex-row align-items-center text-center" data-id="{{ $product->id }}" data-name="{{ $product->name }}" data-price="{{ $product->unit_price ?? 0 }}" data-image="{{ getValidImage(path: 'storage/app/public/product/thumbnail/' . $product->image, type: 'product') }}" style="box-shadow: 0 2px 6px rgba(0,0,0,0.1); transition: all 0.2s ease-in-out;">
                            <div class="mb-2" style="height:120px; display:flex; align-items:center; justify-content:center; width:100%;">
                                <img src="{{ getValidImage(path: 'storage/app/public/product/thumbnail/' . $product['thumbnail'], type: 'product') }}" alt="{{ $product->name }}" style="max-height:100%; max-width:100%; object-fit:contain;">
                            </div>
                            <div class="d-flex flex-column">
                                ...
                                <div class="mt-auto">
                                    ...
                                    <div class="quantity-counter d-none mt-2 d-flex justify-content-center align-items-center">
        ...
        <div class="container">
            <div class="row align-items-center">
                ...
                <div class="col-12 col-md-6 d-flex gap-2 justify-content-center justify-content-md-end">
                    <a href="{{ route('guruji.individual', ['name' => Str::slug($gurujiname->name)]) }}" class="btn w-100 w-md-auto px-3">
                        ← {{ translate('Back') }}
                    </a>
                    <button class="btn w-100 w-md-auto px-3" id="btnEditInfo" style="background-color: #FF6F00; border-color: #FF6F00;" disabled>
                        {{ translate('Proceed') }} →
                    </button>
                </div>
            </div>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            <div class="form-group">
                                ...
                                <input class="form-control text-align-direction" type="text" name="booking_date" id="datepicker"
                                    placeholder="{{ translate('Enter_your_puja_praforming_date') }}"
                                    autocomplete="off" required>
                                <p id="date-validation" class="text-danger d-none">Please select Puja date</p>
                            </div>
                        </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#datepicker').datepicker({
                ...
                modal: true,
                footer: true,
                minDate: today, // aaj se pehle disable
                maxDate: maxDate, // aaj se 1 month ke baad disable
                todayHighlight: true
            });
        ...
        function datePicker() {
            var today = new Date();
            today.setHours(0, 0, 0, 0); // time part reset

            var maxDate = new Date(today);
            maxDate.setMonth(maxDate.getMonth() + 1);

            // Agar day overflow hua toh last day of month set karo
            if (maxDate.getDate() !== today.getDate()) {
                maxDate.setDate(0);
            }
        ...
        // Initialize datepicker jab modal open ho
            ...
            fetch(detailsForm.action, {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        document.getElementById('cPackage').textContent = chosen.name || "—";
                        document.getElementById('cPackagePrice').textContent = "₹" + chosen.amount.toLocaleString();
    ...
    <script>
        function fullReset() {
            ...
            $('#footerProducts').empty();

            if (chosen && chosen.name) {
                $('#footerInfo').html(`<strong>Package:</strong> ${chosen.name} 
            <span style="font-size: 18px; font-weight: bold;">(₹${chosen.amount})</span>`);
            } else {
                $('#footerInfo').html('{{ translate("Select Your Puja Package") }}');
            }
        }
        ...
        //reset when confirm modal closes
    ...
    <script>
        $('#confirmModal').on('show.bs.modal', function() {
            ...
            // Customer Details
            document.getElementById('cName').textContent = $('#person-name').val().trim() || "—";
            document.getElementById('cMobile').textContent = $('#person-number').val().trim() || "—";
            ...
            if (!productRows) {
                productRows = `<tr><td colspan="3" class="text-center text-muted">No products added</td></tr>`;
            }
            document.getElementById("cProducts").innerHTML = productRows;

            // Total Amount
            const totalAmount = chosen.amount + Object.values(selectedProducts).reduce((sum, p) => sum + p.price * p.qty, 0);
            document.getElementById('cAmount').textContent = "₹" + totalAmount.toLocaleString();
        });
    </script>
    ...
    <script>
        $(document).ready(function() {
            ...
            $("#bookNowBtn").on("click", function(e) {
                ...
                // DATE 
                if (date === "") {
                    $("#date-validation").removeClass("d-none");
                    valid = false;
                } else {
