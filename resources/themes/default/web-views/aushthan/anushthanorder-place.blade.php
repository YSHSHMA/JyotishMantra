                    ...
                    @if(auth('customer')->check() || session('guest_id'))
                        <div class="card-body">
                            ...
                            <div class="row mt-4">
                                ...
                                <a href="{{ auth('customer')->check() ? route('account-service-order-details', ['order_id' => $orderId]) : route('customer.auth.login') }}" class="btn btn--primary mb-3 text-center">{{ translate('track_order') }}</a>
