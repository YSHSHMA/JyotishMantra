...
$coupon_dis = 0;
@endphp
@if (session()->has('coupon_discount_instanceanushthan'))
@php
$couponDiscount = session()->has('coupon_discount_instanceanushthan') ? session('coupon_discount_instanceanushthan') :
0;
@endphp
<div class="d-flex justify-content-between">
    <span class="cart_title">{{ translate('coupon_discount_instanceanushthan') }}</span>
...
<div class="pt-2">
    <div class="d-flex align-items-center form-control rounded-pill pl-3 p-1">
        ...
        <div class="px-2 d-flex justify-content-between w-100">
            <div>
                {{ session('coupon_code_instanceanushthan') }}
            ...
            <div class="bg-transparent text-danger cursor-pointer px-2 get-view-by-onclick"
                data-link="{{ route('coupon.poojaremovecoupon', ['code' => session('coupon_code_instanceanushthan')]) }}">
                x</div>
        </div>
    </div>
</div>
...
{{-- <div class="row"> --}}
        ...
        <form class="needs-validation" action="javascript:" method="post" novalidate id="coupon-code-pooja-ajax">
            <div class="d-flex form-control rounded-pill ps-3 p-1">
                <img width="24" src="{{ theme_asset(path: 'public/assets/front-end/img/icons/coupon.svg') }}"
                    alt="" onclick="instanceAnushthanCouponList()">
    ...
    {{-- <div class="col-md-1 pt-2">
        <button class="btn btn--primary btn-sm rounded-pill" onclick="instanceAnushthanCouponList()"><i
...
<div class="d-flex justify-content-between">
    ...
    <span class="cart_value" id="mainProductPrice">
        ...
        @endphp
        @if ($customer->wallet_balance < $productTotalAmount) {{ webCurrencyConverter(amount: $couponTotalAmount) }}
            @else {{ webCurrencyConverter(0.0) }} @endif </span>
</div>
@else
<div class="d-flex justify-content-between">
    <span class="cart_title text-primary font-weight-bold">Total</span>
    <span class="cart_value" id="mainProductPrice">
        @if ($customer->wallet_balance < $productTotalAmount)
            {{ webCurrencyConverter($leadsGet['package_price'] + $final_price_val - $customer->wallet_balance) }} @else
            {{ webCurrencyConverter(0.0) }} @endif </span>
</div>
@endif
    ...
    <div class="Details">
        ...
        <label class="d-flex align-items-center gap-2 mb-0 form-check py-2 cursor-pointer">
            ...
            <span class="text-capitalize form-check-label" hidden>
                @if ($payment_gateway->additional_data && json_decode($payment_gateway->additional_data)->gateway_title
                != null)
        ...
        @if (session()->has('coupon_code_instanceanushthan'))
        @php
        $couponTotalAmount =
        $leadsGet['package_price'] + $final_price_val - $couponDiscount - $customer->wallet_balance;
        @endphp
        @if ($customer->wallet_balance < $productTotalAmount) <input type="hidden" name="payment_amount"
            id="mainProductPriceInput" value="{{ $couponTotalAmount }}">
            @else
            <input type="hidden" name="payment_amount" id="mainProductPriceInput" value="{{ 0.0 }}">
            @endif
            @else
            @if ($customer->wallet_balance < $productTotalAmount) <input type="hidden" name="payment_amount"
