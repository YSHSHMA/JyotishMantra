    ...
    <div class="search-page-header">
        ...
        <form id="search-form" class="d-none d-lg-block" action="{{ route('products') }}" method="GET">
            ...
            <div class="sorting-item">
                ...
                <select class="product-list-filter-on-viewpage1">
                    ...
                    <div class="text-center">
                        ...
                        <div class="__p-25-10 w-100 pt-4">
                            <label class="w-100 opacity-75 text-nowrap for-sorting d-block mb-0 ps-0" for="sorting">
                                <select class="form-control custom-select" id="searchByFilterValue">
                                    ...
                                    <option
                                        value="{{route('products-slug',['slug'=> $data['slug']??'','data_from'=>'best-selling','page'=>1])}}" {{isset($data['data_from'])!=null?$data['data_from']=='best-selling'?'selected':'':''}}>{{translate('best_selling_product')}}</option>
                                    <option
                        ...
                        <div class="__p-25-10 w-100 pt-0 d-lg-none">
                            <form id="search-form" action="{{ route('products') }}" method="GET">
                                <input hidden name="data_from" value="{{$data['data_from']}}">
                                <select class="form-control product-list-filter-on-viewpage1">
                    ...
                    <div class="text-center">
                        ...
                        <ul id="lista1" class="__brands-cate-wrap" data-simplebar
                            ...
                            <div
                                ...
                                id="brand">
                                <li class="flex-between __inline-39 get-view-by-onclick" data-link="{{ route('products-slug',['slug'=> $brand['slug'],'data_from'=>'brand','page'=>1]) }}">
...
<script>
    ...
    function loadMoreProducts(reset = false) {
        ...
        /* if (currentSort) currentUrl.searchParams.set('sort', currentSort); */
        let data_getv = $('#products-search-data-backup');
        $.ajax({
            url: "?",
            data: {
                id: data_getv.data('id'),
                slug: data_getv.data('slug'),
                name: data_getv.data('name'),
                data_from: data_getv.data('from'),
                min_price: data_getv.data('min-price'),
                max_price: data_getv.data('max-price'),
                sort_by: $('.product-list-filter-on-viewpage1').val(),
                page: page,
            },
            type: 'GET',
            success: function(response) {
                if (response.view && $.trim(response.view) !== '') {
                    if (reset) $('#ajax-products').html('');
                    if (response.view.includes('no_product_found')) {
                        hasMorePages = false;
                        $('#no-more-products').html(response.view);
                    } else {
                        $('#ajax-products').append(response.view);
    ...
    function observeTrigger() {
        const target = document.querySelector('#product-end-trigger');
        if (!target) return;
    ...
    document.addEventListener('DOMContentLoaded', function() {
        observeTrigger();
        $('.product-list-filter-on-viewpage1').on('change', function() {
