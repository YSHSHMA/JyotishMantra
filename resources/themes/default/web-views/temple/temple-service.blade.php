...
<body class="bg-gradient-to-b from-orange-400 via-orange-600 to-orange-800 min-h-screen font-sans">
    ...
    <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse" id="mainNavbar">
            ...
            <ul class="navbar-nav mx-auto mb-2 mb-lg-0">
                ...
                <li class="nav-item">
                    <a class="nav-link" href="{{ url('/terms-and-conditions') }}">Terms & Conditions</a>
                </li>

            </ul>

            <!-- LANGUAGE SWITCH (INSIDE NAVBAR NEXT TO MENU) -->
            @php
            ...
            <ul class="navbar-nav mb-2 mb-lg-0 ms-lg-3">
                <li class="nav-item d-flex align-items-center">
                    <div class="form-check form-switch m-0">
                        <input class="form-check-input changeLanguageSelect" type="checkbox"
                               data-action="{{ route('change-language') }}"
                               {{ $currentLang == 'in' ? 'checked' : '' }}>
                        <label class="form-check-label ms-1">
                            <span id="langLabel">{{ $currentLang == 'in' ? 'Hindi' : 'English' }}</span>
                        </label>
                    </div>
                </li>
            </ul>
    ...
    {{-- temple info --}}
    <section id="heroSlider" style="position: relative;">
...
<style>
    ...
    @media(max-width:768px){
        ...
        .hero-slider .item img { height: 240px; }
...
<div class="hero-slider owl-carousel owl-theme">
    @foreach ($images as $photo)
        <div class="item">
            <img src="{{ getValidImage('storage/app/public/temple/gallery/' . $photo, type: 'product') }}" alt="">
        </div>
    @endforeach
</div>
...
<div class="slider-content">
    <h2>{{ $temple['name'] }}</h2>
    <p>{!! Str::limit(strip_tags($temple['details']), 150) !!}</p>
</div>

</section>

<section id="templeInfo">
            ...
            <div class="d-flex justify-content-between flex-wrap gap-3">
                ...
                <div>
                    ...
                    <p class="mb-0">
                        ...
                        {!! Str::limit(strip_tags($temple['details']), 350) !!}
                    </p>
                </div>
                ...
                <div class="text-end">
                    ...
                    <div>
                        @for ($i = 1; $i <= 5; $i++)
                            <i class="{{ $i <= 3 ? 'fa-solid fa-star text-warning' : 'fa-regular fa-star text-muted' }}"></i>
                        @endfor
                        <span>(3.0 / 5)</span>
                    </div>
                </div>
                                    ...
                                    <div class="row g-2" id="inputRow">
                                        ...
                                        <div id="aadhaar-section" class="col-12 d-flex">
                                            <input type="number" id="aadhaar"
                                                ...
                                                placeholder="{{ translate('12_-_digit_Aadhaar') }} {{$temple->aadhaar_verify_status==1?'Required':'Optional'}}" maxlength="12" pattern="\d{12}"
    ...
    <script>
        ...
        // show already filled detail on tab
        function getFilledDetail() {
                ...
                success: function(response) {
                    if (response.status === 'success' && response.details && response.details.length >
                        ...
                        leadDataArray.forEach(leadData => {
                            const type = leadData.type?.toLowerCase();
                            foundTypes.push(type);
                            const html = buildLeadFilledHtml(leadData);
                            $(`#nav-${type}`).html(html);
                            $(`#nav-${type}-tab`).addClass('bg-success text-white');
                            allLeadDataForSummary.push(leadData);
                        });

                        renderFilledSummaryPopup(allLeadDataForSummary);
            ...
            function renderFilledSummaryPopup(leadDataArray) {
                let totalAmount = 0;
                let summaryHtml = `
                <div>
                    <h6 class="text-primary mb-2">
            ...
            function buildLeadFilledHtml(leadData) {
                const templeName = "{{ $temple['name'] ?? 'Temple Name' }}";
                let customersList = '-';
                let lockerList = '-';
                const customerQty = parseInt(leadData.customer_qty || 1);
    ...
    <script>
        function darshanNextDateBtn() {
            var darshanDate = $('#vipDate-select').val();
            var pujaDate = $('#pujaDate').val();
            var bhojanDate = $('#bhojanDate').val();
            var lockerDate = $('#lockerDate').val();

            if (darshanDate) {
                let d = new Date(darshanDate);
                d.setDate(d.getDate() + 1);
                $('#vipDate-select').val(d.toISOString().split("T")[0]);
                loadTimeSlotsGeneric('darshan');
            }

            if (pujaDate) {
                let p = new Date(pujaDate);
                p.setDate(p.getDate() + 1);
                $('#pujaDate').val(p.toISOString().split("T")[0]);
            }

            if (bhojanDate) {
                let b = new Date(bhojanDate);
                b.setDate(b.getDate() + 1);
                $('#bhojanDate').val(b.toISOString().split("T")[0]);
                loadTimeSlotsGeneric('bhojan');
            }

            if (lockerDate) {
                let l = new Date(lockerDate);
                l.setDate(l.getDate() + 1);
                $('#lockerDate').val(l.toISOString().split("T")[0]);
            }
        }

        function pujaNextDateBtn() {
            var pujaDate = $('#pujaDate').val();
            if (pujaDate) {
                let p = new Date(pujaDate);
                p.setDate(p.getDate() + 1);
    ...
    <script>
        ...
        $(document).on('change',
            ...
            function() {
                ...
                if (darshanDate) {
                    $('#vipDate-select').val(darshanDate);
                    loadTimeSlotsGeneric('darshan');
                }

                if (pujaDate) {
                    $('#pujaDate').val(darshanDate);
                }

                if (bhojanDate) {
                    $('#bhojanDate').val(darshanDate);
                    loadTimeSlotsGeneric('bhojan');
                }

                if (lockerDate) {
                    $('#lockerDate').val(darshanDate);
                }
            });

        $(document).on('change',
            '.darshan-package-select, .bhojan-package-select, .bhojan-date-select',
            function() {
    ...
    $(document).on('change', '.changeLanguageSelect', function () {
        ...
        let lang = $(this).is(':checked') ? 'in' : 'en';
        $('#langLabel').text(lang === 'in' ? 'Hindi' : 'English');
